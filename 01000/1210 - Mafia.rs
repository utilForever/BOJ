...
fn main() {
    ...
    for i in 0..n {
        maximum_flow.add_edge(2 * i, 2 * i + 1, costs[i]);
    }

    for &(u, v) in edges.iter() {
        maximum_flow.add_edge(2 * u + 1, 2 * v, INF);
        maximum_flow.add_edge(2 * v + 1, 2 * u, INF);
    }
    ...
    for i in 0..n {
        if maximum_flow.check[2 * i] >= 0 && maximum_flow.check[2 * i + 1] < 0 {
            ret.push(i + 1);
        }
    }
