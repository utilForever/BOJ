...
fn main() {
    ...
    for i in 0..n {
        for j in 0..m {
            let cost_white = (image[i][j] - a).abs();
            let cost_black = (image[i][j] - b).abs();

            maximum_flow.add_edge(source, i * m + j, cost_white);
            maximum_flow.add_edge(i * m + j, sink, cost_black);

            if i + 1 < n {
                let cost2 = (image[i][j] - image[i + 1][j]).abs();

                maximum_flow.add_edge(i * m + j, (i + 1) * m + j, cost2);
                maximum_flow.add_edge((i + 1) * m + j, i * m + j, cost2);
            }

            if j + 1 < m {
                let cost2 = (image[i][j] - image[i][j + 1]).abs();

                maximum_flow.add_edge(i * m + j, i * m + j + 1, cost2);
                maximum_flow.add_edge(i * m + j + 1, i * m + j, cost2);
            }
        }
    }
