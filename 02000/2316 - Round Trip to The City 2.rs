...
fn main() {
    ...
    maximum_flow.add_edge(0, 2, i64::MAX);
    maximum_flow.add_edge(3, 2 * n + 1, i64::MAX);

    for i in 1..=n {
        maximum_flow.add_edge(2 * i - 1, 2 * i, 1);
    }

    for _ in 1..=p {
        let (s, e) = (scan.token::<usize>(), scan.token::<usize>());
        maximum_flow.add_edge(2 * s, 2 * e - 1, 1);
        maximum_flow.add_edge(2 * e, 2 * s - 1, 1);
    }
