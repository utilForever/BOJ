    ...
    for i in 0..n {
        for j in 0..n {
            ...
            maximum_flow.add_edge(
                2 * (i * n + j),
                2 * (i * n + j) + 1,
                if jail[i][j] == '.' { 1 } else { INF },
            );

            for d in 0..4 {
                let y_next = i as isize + DY[d];
                let x_next = j as isize + DX[d];
                ...
                if jail[y_next][x_next] == '+' {
                    continue;
                }

                maximum_flow.add_edge(2 * (i * n + j) + 1, 2 * (y_next * n + x_next), INF);
            }

            if jail[i][j] == 'X' {
                maximum_flow.add_edge(source, 2 * (i * n + j), INF);
            } else if jail[i][j] == 'O' {
                maximum_flow.add_edge(2 * (i * n + j) + 1, sink, INF);
            }
        }
    }
