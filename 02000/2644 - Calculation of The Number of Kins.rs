use io::Write;
use std::{io, str};

pub struct UnsafeScanner<R> {
    reader: R,
    buf_str: Vec<u8>,
    buf_iter: str::SplitAsciiWhitespace<'static>,
}

impl<R: io::BufRead> UnsafeScanner<R> {
    pub fn new(reader: R) -> Self {
        Self {
            reader,
            buf_str: vec![],
            buf_iter: "".split_ascii_whitespace(),
        }
    }

    pub fn token<T: str::FromStr>(&mut self) -> T {
        loop {
            if let Some(token) = self.buf_iter.next() {
                return token.parse().ok().expect("Failed parse");
            }
            self.buf_str.clear();
            self.reader
                .read_until(b'\n', &mut self.buf_str)
                .expect("Failed read");
            self.buf_iter = unsafe {
                let slice = str::from_utf8_unchecked(&self.buf_str);
                std::mem::transmute(slice.split_ascii_whitespace())
            }
        }
    }
}

fn get_length(graph: &Vec<Vec<usize>>, num_vertices: usize, start: usize, end: usize) -> i64 {
    let mut stack = Vec::new();
    let mut check = vec![false; num_vertices + 1];
    let mut ret = -1;

    stack.push((start, 0));
    check[start] = true;

    while !stack.is_empty() {
        let (cur_node, length) = *stack.last().unwrap();
        let mut can_connect = false;

        if cur_node == end {
            ret = length;
            break;
        }

        for vertex in graph[cur_node].iter() {
            if !check[*vertex] {
                stack.push((*vertex, length + 1));
                check[*vertex] = true;
                can_connect = true;
                break;
            }
        }

        if !can_connect {
            stack.pop();
        }
    }

    ret
}

fn main() {
    let (stdin, stdout) = (io::stdin(), io::stdout());
    let mut scan = UnsafeScanner::new(stdin.lock());
    let mut out = io::BufWriter::new(stdout.lock());

    let n = scan.token::<usize>();
    let mut graph = vec![Vec::new(); n + 1];

    let (start, end) = (scan.token::<usize>(), scan.token::<usize>());
    let m = scan.token::<i64>();

    for _ in 0..m {
        let (a, b) = (scan.token::<usize>(), scan.token::<usize>());
        graph[a].push(b);
        graph[b].push(a);
    }

    writeln!(out, "{}", get_length(&graph, n, start, end)).unwrap();
}
