...
fn main() {
    ...
    let mut solders = vec![(0, 0); n];

    for i in 0..n {
        solders[i] = (scan.token::<i64>(), scan.token::<i64>());
    }
    ...
    for i in 0..n {
        if solders[i].1 <= R {
            maximum_flow.add_edge(source, 2 * i, INF);
        }

        if w - solders[i].1 <= R {
            maximum_flow.add_edge(2 * i + 1, sink, INF);
        }

        for j in i + 1..n {
            let dist = (solders[i].0 - solders[j].0).pow(2) + (solders[i].1 - solders[j].1).pow(2);

            if dist <= (2 * R).pow(2) {
                maximum_flow.add_edge(2 * i + 1, 2 * j, INF);
                maximum_flow.add_edge(2 * j + 1, 2 * i, INF);
            }
        }
    }
