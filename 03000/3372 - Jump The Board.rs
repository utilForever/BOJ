...
fn main() {
    ...
    let mut num_path = vec![vec!["0".to_string(); n]; n];
    ...
    for i in 0..n {
        for j in 0..n {
            ...
            if (num + i as i64) < n as i64 {
                num_path[num as usize + i][j] = add(
                    num_path[num as usize + i][j].clone(),
                    num_path[i][j].clone(),
                );
            }

            if (num + j as i64) < n as i64 {
                num_path[i][num as usize + j] = add(
                    num_path[i][num as usize + j].clone(),
                    num_path[i][j].clone(),
                );
            }
        }
