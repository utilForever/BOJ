...
fn main() {
    ...
    for _ in 0..100 {
        ...
        for (i, &(a, b)) in edges.iter().enumerate() {
            maximum_flow.add_edge(source, 1 + i, q);
            maximum_flow.add_edge(1 + i, m + 1 + a, INF);
            maximum_flow.add_edge(1 + i, m + 1 + b, INF);
        }

        for i in 0..n {
            maximum_flow.add_edge(m + 1 + i, sink, p);
        }
        ...
        let g = q * edges_in - p * team.len() as i64;
