...
fn main() {
    ...
    loop {
        let (n, m) = match (
            words.get(idx).and_then(|x| x.parse::<usize>().ok()),
            words.get(idx + 1).and_then(|x| x.parse::<usize>().ok()),
        ) {
            (Some(n), Some(m)) => (n, m),
            _ => break,
        };
        ...
        for i in 0..m {
            let node = words[idx + i];
            let pos_comma = node.find(',').unwrap();
            let u = node[1..pos_comma].parse::<usize>().unwrap();
            let v = node[pos_comma + 1..node.len() - 1]
                .parse::<usize>()
                .unwrap();
        ...
        for s in 0..n {
            for t in s + 1..n {
                let source = s * 2 + 1;
                let sink = t * 2;
                let mut maximum_flow = Dinic::new(2 * n, source, sink);

                for v in 0..n {
                    let capacity = if v == s || v == t { INF } else { 1 };
                    maximum_flow.add_edge(v * 2, v * 2 + 1, capacity);
                }

                for &(u, v) in edges.iter() {
                    maximum_flow.add_edge(u * 2 + 1, v * 2, INF);
                    maximum_flow.add_edge(v * 2 + 1, u * 2, INF);
                }

                ret = ret.min(maximum_flow.get_flow());
            }
        }
