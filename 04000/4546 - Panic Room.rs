...
fn main() {
    ...
    for _ in 0..x {
        ...
        let source = m;
        let sink = n;
        let mut maximum_flow = Dinic::new(m + 1, source, sink);

        for from in 0..m {
            let status = scan.token::<String>();
            ...
            for _ in 0..c {
                let to = scan.token::<usize>();

                maximum_flow.add_edge(from, to, INF);
                maximum_flow.add_edge(to, from, 1);
            }
        }

        for intruder in intruders {
            maximum_flow.add_edge(source, intruder, INF);
        }

        let ret = maximum_flow.get_flow();

        if ret >= INF / 2 {
            writeln!(out, "PANIC ROOM BREACH").unwrap();
        } else {
