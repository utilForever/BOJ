...
fn main() {
    ...
    loop {
        ...
        unsafe {
            ...
            let mut planes = Vec::with_capacity(convex_hull3.faces.len());

            for face in convex_hull3.faces.iter() {
                let a = points[(*(*face)).a];
                let b = points[(*(*face)).b];
                let c = points[(*(*face)).c];
                ...
                let ret_b = normal.y;
                let ret_c = normal.z;
                let ret_d = -(ret_a * a.x + ret_b * a.y + ret_c * a.z);
                planes.push((ret_a, ret_b, ret_c, ret_d));
            }
            ...
            for _ in 0..q {
                ...
                for plane in planes.iter() {
                    ...
                    ret = ret.min(dist.abs());
                }

                writeln!(out, "{:.4}", ret).unwrap();
            }
        }
    }
}
