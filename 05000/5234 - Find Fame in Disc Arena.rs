...
fn main() {
    ...
    for i in 1..=t {
        ...
        let mut fames = vec![0; k + 1];
        let mut prerequisites = vec![Vec::new(); k + 1];

        for j in 1..=k {
            let (f, d) = (scan.token::<i64>(), scan.token::<usize>());

            fames[j] = f;

            for _ in 0..d {
                prerequisites[j].push(scan.token::<usize>());
            }
        }

        let source = 0;
        let sink = k + 1;
        let mut maximum_flow = Dinic::new(k + 2, source, sink);

        for j in 1..=k {
            if fames[j] > 0 {
                maximum_flow.add_edge(source, j, fames[j]);
            } else {
                maximum_flow.add_edge(j, sink, -fames[j]);
            }

            for &prerequisite in prerequisites[j].iter() {
                maximum_flow.add_edge(j, prerequisite, INF);
            }
        }

        writeln!(
            out,
            "Case {i}: Maximum attainable fame = {}",
            fames.iter().filter(|&&f| f > 0).sum::<i64>() - maximum_flow.get_flow()
        )
        .unwrap();
    }
}
