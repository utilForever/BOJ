...
fn main() {
    ...
    for _ in 0..t {
        let (nr, nc) = (scan.token::<usize>(), scan.token::<usize>());
        let (er, ec) = (scan.token::<usize>(), scan.token::<usize>());
        let (kr, kc) = (scan.token::<usize>(), scan.token::<usize>());

        let source = nc * er + ec;
        let sink = nc * kr + kc;
        let mut maximum_flow = Dinic::new(nr * nc, source, sink);
        ...
        for i in 0..nr {
            for j in 0..nc - 1 {
                ...
                if area > 0 {
                    maximum_flow.add_edge(i * nc + j, i * nc + j + 1, area + 1);
                    maximum_flow.add_edge(i * nc + j + 1, i * nc + j, area + 1);
                }
            }
        }
        ...
        for i in 0..nr - 1 {
            for j in 0..nc {
                ...
                if area > 0 {
                    maximum_flow.add_edge(i * nc + j, (i + 1) * nc + j, area + 1);
                    maximum_flow.add_edge((i + 1) * nc + j, i * nc + j, area + 1);
                }
            }
        }
