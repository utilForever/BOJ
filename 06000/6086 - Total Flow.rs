...
fn main() {
    ...
    let mut maximum_flow = MaximumFlow::new(54, 0, 53);

    maximum_flow.add_edge(0, 27, i64::MAX);
    maximum_flow.add_edge(52, 53, i64::MAX);
    ...
    for _ in 0..n {
        ...
        let from = if from.is_uppercase() {
            from as usize - 'A' as usize + 27
        } else {
            from as usize - 'a' as usize + 1
        };
        let to = if to.is_uppercase() {
            to as usize - 'A' as usize + 27
        } else {
            to as usize - 'a' as usize + 1
        };

        maximum_flow.add_edge(from, to, capacity);
        maximum_flow.add_edge(to, from, capacity);
    }
