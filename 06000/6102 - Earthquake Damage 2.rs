...
fn main() {
    ...
    let mut cowpaths = vec![(0, 0); c];

    for i in 0..c {
        cowpaths[i] = (scan.token::<usize>(), scan.token::<usize>());
    }

    let mut reports = vec![0; n];
    let mut is_reported = vec![false; p + 1];

    for i in 0..n {
        let report = scan.token::<usize>();

        reports[i] = report;
        is_reported[report] = true;
    }

    let source = 0;
    let sink = 2 * p;
    let mut maximum_flow = Dinic::new(2 * p + 1, source, sink);

    for i in 1..=p {
        let capacity = if i == 1 || is_reported[i] { INF } else { 1 };
        maximum_flow.add_edge(2 * (i - 1), 2 * (i - 1) + 1, capacity);
    }

    for (u, v) in cowpaths {
        maximum_flow.add_edge(2 * (u - 1) + 1, 2 * (v - 1), INF);
        maximum_flow.add_edge(2 * (v - 1) + 1, 2 * (u - 1), INF);
    }

    for report in reports {
        maximum_flow.add_edge(2 * (report - 1) + 1, sink, INF);
    }
