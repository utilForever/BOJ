...
fn main() {
    ...
    loop {
        ...
        let mut count = vec![0_i64; 200_001];
        let mut max_count = vec![0_i64; 200_001];
        let mut ans = vec![0_i64; q + 1];
        let mut ans_query = 0;

        for i in 1..=q {
            let l1 = query[block[i - 1].1].0 as i64;
            let r1 = query[block[i - 1].1].1 as i64;
            let l2 = query[block[i].1].0 as i64;
            let r2 = query[block[i].1].1 as i64;
