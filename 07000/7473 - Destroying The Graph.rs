...
fn main() {
    ...
    let mut dollars_plus = vec![0; n];
    let mut dollars_minus = vec![0; n];

    for i in 0..n {
        dollars_plus[i] = scan.token::<i64>();
    }

    for i in 0..n {
        dollars_minus[i] = scan.token::<i64>();
    }
    ...
    for i in 0..n {
        maximum_flow.add_edge(source, i, dollars_minus[i]);
        maximum_flow.add_edge(i + n, sink, dollars_plus[i]);
    }

    for _ in 0..m {
        let (u, v) = (scan.token::<usize>() - 1, scan.token::<usize>() - 1);
        maximum_flow.add_edge(u, v + n, INF);
    }
