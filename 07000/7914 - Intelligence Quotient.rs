...
fn main() {
    ...
    let mut acquaintances = vec![vec![false; m]; n];

    for _ in 0..k {
        let (a, b) = (scan.token::<usize>() - 1, scan.token::<usize>() - 1);
        acquaintances[a][b] = true;
    }

    let mut iqs_math = vec![0; n];
    let mut iqs_cs = vec![0; m];

    for i in 0..n {
        iqs_math[i] = scan.token::<i64>();
    }

    for i in 0..m {
        iqs_cs[i] = scan.token::<i64>();
    }
    ...
    for i in 0..n {
        maximum_flow.add_edge(source, i + 1, iqs_math[i]);
    }

    for i in 0..m {
        maximum_flow.add_edge(n + 1 + i, sink, iqs_cs[i]);
    }

    for i in 0..n {
        for j in 0..m {
            if !acquaintances[i][j] {
                maximum_flow.add_edge(i + 1, n + 1 + j, INF);
            }
        }
    }

    let flow = maximum_flow.get_flow();
    let ret = iqs_math.iter().sum::<i64>() + iqs_cs.iter().sum::<i64>() - flow;
    let reachable = maximum_flow.reachable_from_source();

    writeln!(out, "{ret}").unwrap();

    let mut ret_math = Vec::new();
    let mut ret_cs = Vec::new();
    ...
    writeln!(out, "{}", ret_math.len()).unwrap();
