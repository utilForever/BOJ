...
fn main() {
    ...
    let mut froms = vec![false; n + 1];
    let mut tos = vec![false; n + 1];
    ...
    let mut idxes_froms = vec![0; n + 1];
    let mut idxes_tos = vec![0; n + 1];
    let mut idx = 1;
    ...
    let source = 0;
    let sink = idx;
    let mut maximum_flow = Dinic::new(idx + 1, source, sink);

    if edges.iter().any(|&(a, b)| a == 1 && b == n) {
        maximum_flow.add_edge(source, sink, 1);
    }

    for i in 1..=n {
        if froms[i] {
            maximum_flow.add_edge(source, idxes_froms[i], 1);
        }

        if tos[i] {
            maximum_flow.add_edge(idxes_tos[i], sink, 1);
        }

        if froms[i] && tos[i] {
            maximum_flow.add_edge(idxes_froms[i], idxes_tos[i], INF);
        }
    }

    for &(a, b) in edges.iter() {
        if froms[a] && tos[b] {
            maximum_flow.add_edge(idxes_froms[a], idxes_tos[b], INF);
        }
    }
