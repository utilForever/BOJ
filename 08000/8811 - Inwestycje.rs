...
fn main() {
    ...
    for _ in 0..z {
        ...
        let mut costs = vec![0; a];
        let mut values = vec![0; b];
        ...
        let source = 0;
        let sink = a + b + 1;
        let mut maximum_flow = Dinic::new(a + b + 2, source, sink);

        for i in 0..b {
            maximum_flow.add_edge(source, i + 1, values[i]);
        }

        for i in 0..a {
            maximum_flow.add_edge(i + b + 1, sink, costs[i]);
        }
        ...
        for _ in 0..k {
            ...
            maximum_flow.add_edge(y, b + x, INF);
        }

        let sum = values.iter().sum::<i64>();
        let flow = maximum_flow.get_flow();

        writeln!(out, "{}", sum - flow).unwrap();
    }
}
