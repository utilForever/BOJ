...
fn is_point_on_segment(p: Point, a: Point, b: Point) -> bool {
    ...
    if cross.abs() > EPS || dot < 0.0 || dot > squared_length_ab {
        false
    } else {
        true
    }
...
fn intersect_segment_open_rectangle(
    a: Point,
    b: Point,
    y1: f64,
    y2: f64,
    ...
    if !clip(-dy, a.y - y_min) {
        return false;
    }

    if !clip(dy, y_max - a.y) {
        return false;
    }

    if !clip(-dx, a.x - x_min) {
        return false;
    }

    if !clip(dx, x_max - a.x) {
        return false;
    }
...
fn check(polygon: &Vec<Point>, y1: f64, y2: f64, x1: f64, x2: f64) -> bool {
    for &point in polygon.iter() {
        if point.x > x1 + EPS && point.x < x2 - EPS && point.y > y1 + EPS && point.y < y2 - EPS {
            return true;
        }
    }
    ...
    for i in 0..n {
        ...
        if intersect_segment_open_rectangle(a, b, y1, y2, x1, x2) {
            return true;
        }
    }
...
fn calculate(polygon: &Vec<Point>, width: f64, height: f64) -> i64 {
...
fn main() {
    ...
    for offset in offsets_candidate {
        ...
        let cnt = calculate(&translated, w, h);
        ret = ret.min(cnt);
    }
