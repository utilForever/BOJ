...
fn main() {
    ...
    for _ in 0..n {
        let (m, r) = (scan.token::<usize>(), scan.token::<f64>() / 2.0);
        let mut points = vec![(0.0, 0.0); m];
        ...
        for i in 0..m {
            for j in 0..m {
                ...
                // Can't exceed 2.0 * r because the radius of the circle is r
                if dist_points > 2.0 * r + 1e-5 {
                    continue;
                }

                // Calculate the height of the triangle by using the Pythagorean theorem
                let len_height = (r * r - dist_points * dist_points / 4.0).sqrt();
