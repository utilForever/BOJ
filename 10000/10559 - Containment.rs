...
fn main() {
    ...
    let source = SIZE * SIZE * SIZE;
    let sink = SIZE * SIZE * SIZE + 1;

    let idx = |x: usize, y: usize, z: usize| -> usize { x * SIZE * SIZE + y * SIZE + z };
    let mut maximum_flow = Dinic::new(SIZE * SIZE * SIZE + 2, source, sink);
    ...
    for x in 0..SIZE {
        for y in 0..SIZE {
            for z in 0..SIZE {
                ...
                if cnt_faces > 0 {
                    maximum_flow.add_edge(u, sink, cnt_faces);
                }
    ...
    for _ in 0..t {
        ...
        let mut maximum_flow = maximum_flow.clone();
        ...
        for _ in 0..n {
            ...
            maximum_flow.add_edge(source, idx(x, y, z), INF);
        }
