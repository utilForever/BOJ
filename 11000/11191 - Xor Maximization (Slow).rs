...
impl Matrix {
    ...
    fn new_with_size_elements(n: usize, m: usize, elements: Vec<Vec<i64>>) -> Self {
        Self {
            n,
            m,
            column_idx: (0..m).collect(),
    ...
    fn gauss_elimination(&self, modular: i64) -> Matrix {
        let mut ret = Matrix::new_with_size_elements(self.n, self.m, self.elements.clone());

        let mut j = 0;
        let mut r = 0;

        while j < self.m && r < self.n {
            for i in r..self.n {
                if ret[(i, j)] != 0 {
                    ret.row_swap(r, i);
                    break;
            ...
            if ret[(r, j)] != 0 {
                ret.row_mul(r, pow(ret[(r, j)], modular - 2, modular), modular);

                for i in r + 1..self.n {
                    ret.row_add(i, r, modular - ret[(i, j)], modular);
                }
    ...
    fn rref(&self, modular: i64) -> Matrix {
        let mut ret = Matrix::new_with_size_elements(self.n, self.m, self.elements.clone());
        let mut j = 0;
        let mut r = 0;
...
fn main() {
    ...
    let mut matrix = Matrix::new(n, 61);

    for i in 0..n {
        let val = scan.token::<i64>();

        for j in 0..=60 {
            matrix[(i, j)] = ((val >> j) & 1) as i64;
        }
    }

    let rref = matrix.rref(2);
    let mut rows = Vec::<i64>::new();
