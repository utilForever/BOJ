...
fn chinese_remainder_theorem(a: i64, b: i64, m: i64, n: i64) -> i64 {
    let (a, b, m, n) = (a as i128, b as i128, m as i128, n as i128);
    let (_, x, y) = process_extended_euclidean(m, n);
    let k = m * n;
...
fn main() {
    ...
    let c = ntt2::polymul(&a, &b, 998244353);
    let d = ntt2::polymul(&a, &b, 1092616193);
    let mut ret = vec![0; n + m - 1];

    for i in 0..n + m - 1 {
        ret[i] = chinese_remainder_theorem(c[i] as i64, d[i] as i64, 998244353, 1092616193);
    }

    writeln!(
        out,
        "{}",
        ret.iter().skip(1).fold(ret[0], |acc, &x| acc ^ x)
    )
    .unwrap();
}
