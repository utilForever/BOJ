...
fn process_dfs(
    ...
    for vertex in graph[cur_idx].iter() {
        if *vertex != parent_idx {
            size[cur_idx] += process_dfs(graph, parent, size, *vertex, cur_idx);
        }
    }
...
fn process_hld(
    ...
    for vertex in graph[cur_idx].iter() {
        if *vertex != parent_idx && (heavy == -1 || size[*vertex] > size[heavy as usize]) {
            heavy = *vertex as i64;
        }
    }
    ...
    for vertex in graph[cur_idx].iter() {
        if *vertex != parent_idx && *vertex as i64 != heavy {
            process_hld(
                graph,
                size,
                depth,
...
fn main() {
    ...
    process_dfs(&graph, &mut parent, &mut size, 1, 0);
    ...
    for _ in 0..m {
        ...
        writeln!(
            out,
            "{}",
            find_lca(&parent, &depth, &chain_number, &chain_index, a, b)
        )
        .unwrap();
    }
}
