...
fn main() {
    ...
    for _ in 0..t {
        ...
        for _ in 0..5 {
            cards_raw_player1.push(scan.token::<String>());
        }

        for _ in 0..5 {
            cards_raw_player2.push(scan.token::<String>());
        }
        ...
        match hand1.partial_cmp(&hand2) {
            Some(order) => match order {
                Ordering::Less => {
                    for card in cards_raw_player2.iter().enumerate() {
                        if card.0 != 0 {
                            write!(out, " ").unwrap();
                        }

                        write!(out, "{}", card.1).unwrap();
                    }
                    writeln!(out).unwrap();
                }
                Ordering::Greater => {
                    for card in cards_raw_player1.iter().enumerate() {
                        if card.0 != 0 {
                            write!(out, " ").unwrap();
                        }
