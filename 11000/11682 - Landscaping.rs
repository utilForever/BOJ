...
fn main() {
    ...
    for i in 0..n {
        for j in 0..m {
            match field[i][j] {
                '.' => {
                    maximum_flow.add_edge(source, i * m + j, b);
                    maximum_flow.add_edge(i * m + j, sink, 0);
                }
                '#' => {
                    maximum_flow.add_edge(source, i * m + j, 0);
                    maximum_flow.add_edge(i * m + j, sink, b);
                }
                _ => unreachable!(),
            }
        }
    ...
    for i in 0..n {
        for j in 0..m {
            if i + 1 < n {
                maximum_flow.add_edge_with_rev(i * m + j, (i + 1) * m + j, a, a);
            }

            if j + 1 < m {
                maximum_flow.add_edge_with_rev(i * m + j, i * m + (j + 1), a, a);
            }
        }
    }
