...
fn main() {
    ...
    for i in 1..=t {
        ...
        for j in 0..n {
            if towers[j].3 > 0 {
                maximum_flow.add_edge(source, j, towers[j].3);
                sum += towers[j].3;
            } else if towers[j].3 < 0 {
                maximum_flow.add_edge(j, sink, -towers[j].3);
            }
        }
        ...
        for j in 0..n {
            for k in 0..n {
                ...
                let dx = towers[j].0 - towers[k].0;
                let dy = towers[j].1 - towers[k].1;

                if dx * dx + dy * dy <= towers[j].2 * towers[j].2 {
                    maximum_flow.add_edge(j, k, INF);
                }
            }
        }
        ...
        writeln!(out, "Case #{i}: {}", sum - flow).unwrap();
    }
}
