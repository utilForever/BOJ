...
fn main() {
    ...
    for idx in 1..=t {
        ...
        for i in 0..n {
            for j in 0..m {
                if calendar[i][j] == '#' {
                    ...
                    if i + 1 < n && calendar[i + 1][j] == '#' {
                        base -= 2;
                    }

                    if j + 1 < m && calendar[i][j + 1] == '#' {
                        base -= 2;
                    }
                }
            }
        ...
        let mut idxes = vec![vec![usize::MAX; m]; n];
        let mut cnt_question = 0;

        for i in 0..n {
            for j in 0..m {
                if calendar[i][j] == '?' {
                    idxes[i][j] = cnt_question;
                    cnt_question += 1;
                }
            }
        }

        let source = cnt_question;
        let sink = cnt_question + 1;
        let mut maximum_flow = MaximumFlow::new(cnt_question + 2, source, sink);
        ...
        for i in 0..n {
            for j in 0..m {
                ...
                for k in 0..4 {
                    ...
                    if calendar[y_next][x_next] == '#' {
                        deg_blue += 1;
                    }
                }

                let profit = 4 - 2 * deg_blue;
                let is_black = ((i + j) & 1) == 0;

                if is_black {
                    if profit >= 0 {
                        maximum_flow.add_edge(source, idxes[i][j], profit);
                        sum_positive += profit;
                    } else {
                        maximum_flow.add_edge(idxes[i][j], sink, -profit);
                    }
                } else {
                    if profit >= 0 {
                        maximum_flow.add_edge(idxes[i][j], sink, profit);
        ...
        for i in 0..n {
            for j in 0..m {
                ...
                for k in 0..4 {
                    ...
                    if calendar[y_next][x_next] == '?' {
                        maximum_flow.add_edge(idxes[i][j], idxes[y_next][x_next], 2);
                    }
                }
            }
        }
        ...
        let ret = base + sum_positive - flow;

        writeln!(out, "Case #{idx}: {ret}").unwrap();
    }
}
