...
fn main() {
    ...
    for i in 1..=n {
        ...
        let mut chessboard: Vec<Vec<i64>> = vec![vec![-1; c]; r];
        ...
        for j in 0..r {
            ...
            for (k, c) in line.chars().enumerate() {
                ...
                chessboard[j][k] = cnt_vertex;
                cnt_vertex += 1;
            }
        }

        let mut graph = vec![Vec::new(); cnt_vertex as usize];
        let mut cnt_edge = 0;
        ...
        for y in 0..r {
            for x in 0..c {
                ...
                let idx_curr = chessboard[y][x] as usize;

                for d in 0..8 {
                    let y_next = y as i64 + dy[d];
                    let x_next = x as i64 + dx[d];
                    ...
                    if chessboard[y_next][x_next] == -1 {
                        continue;
                    }

                    let idx_next = chessboard[y_next][x_next] as usize;

                    if !graph[idx_curr].contains(&idx_next) {
                        graph[idx_curr].push(idx_next);
                        cnt_edge += 1;
                    }

                    if !graph[idx_next].contains(&idx_curr) {
                        graph[idx_next].push(idx_curr);
                        cnt_edge += 1;
                    }
                }
            }
        ...
        let mut edges_with_king = Vec::with_capacity(cnt_edge);

        for j in 0..graph.len() {
            for k in 0..graph[j].len() {
                edges_with_king.push((j, graph[j][k], 1));
            }
        }

        let matching_with_king = Matching::new(edges_with_king).solve();

        let mut idxes_new = vec![-1; cnt_vertex as usize];
        let mut cnt_vertex_new = 0;

        for j in 0..cnt_vertex {
            if j == idx_king {
                continue;
            }

            idxes_new[j as usize] = cnt_vertex_new as i32;
            cnt_vertex_new += 1;
        }

        let mut graph_new = vec![Vec::new(); cnt_vertex_new];
        let mut cnt_edge_new = 0;
        ...
        for j in 0..cnt_vertex {
            ...
            let j_new = idxes_new[j as usize] as usize;

            for k in 0..graph[j as usize].len() {
                if graph[j as usize][k] == idx_king as usize {
                    continue;
                }

                let k_new = idxes_new[graph[j as usize][k]] as usize;

                if !graph_new[j_new].contains(&k_new) {
                    graph_new[j_new].push(k_new);
                    cnt_edge_new += 1;
                }
            }
        }

        let mut edges_without_king = Vec::with_capacity(cnt_edge_new);

        for j in 0..graph_new.len() {
            for k in 0..graph_new[j].len() {
                edges_without_king.push((j, graph_new[j][k], 1));
            }
        }

        let matching_without_king = Matching::new(edges_without_king).solve();

        let ret_with_king = matching_with_king
            .iter()
            .filter(|&&x| x != SENTINEL)
            .count()
            / 2;
        let ret_without_king = matching_without_king
            .iter()
            .filter(|&&x| x != SENTINEL)
            .count()
            / 2;
        ...
        writeln!(
            out,
            "Case #{i}: {}",
            if ret_with_king == ret_without_king + 1 {
                "A"
            } else {
                "B"
            }
