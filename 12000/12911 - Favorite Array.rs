...
fn generate_initial_terms(sums: &Vec<i64>, m: usize) -> Vec<i64> {
    let n = sums.len();
...
fn main() {
    ...
    let initial_terms = generate_initial_terms(&sums, 2 * len + 1);
    let ret = guess_nth_term(initial_terms, n - 1, MOD);
