...
fn main() {
    ...
    let mut energies = vec![0; n];

    for i in 0..n {
        energies[i] = scan.token::<i64>();
    }
    ...
    for i in 0..m {
        ...
        maximum_flow.add_edge(source, i + 1, c);
        maximum_flow.add_edge(i + 1, m + a, INF);
        maximum_flow.add_edge(i + 1, m + b, INF);
    }

    for i in 0..n {
        maximum_flow.add_edge(m + 1 + i, sink, energies[i]);
    }

    writeln!(out, "{}", sum - maximum_flow.get_flow()).unwrap();
}
