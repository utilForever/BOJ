...
fn main() {
    ...
    let mut melodies = vec![0; n];

    for i in 0..n {
        melodies[i] = scan.token::<usize>();
    }
    ...
    let mut cnt = vec![vec![0; m + 1]; m + 1];

    for i in 0..n - 1 {
        let mut a = melodies[i];
        let mut b = melodies[i + 1];
    ...
    let source = 0;
    let sink = m + 1;
    let mut maximum_flow = Dinic::new(m + 2, source, sink);

    for i in 1..=m {
        for j in i + 1..=m {
            if cnt[i][j] > 0 {
                maximum_flow.add_edge(i, j, cnt[i][j]);
                maximum_flow.add_edge(j, i, cnt[i][j]);
            }
        }
    }
    ...
    for i in high + 1..=m {
        maximum_flow.add_edge(i, sink, INF);
    }
