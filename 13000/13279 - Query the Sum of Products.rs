...
fn divide_and_conquer(nums: &Vec<Vec<usize>>, left: usize, right: usize) -> Vec<usize> {
    if left == right {
        return nums[left].clone();
    }

    let mid = (left + right) / 2;
    let a = divide_and_conquer(nums, left, mid);
    let b = divide_and_conquer(nums, mid + 1, right);

    ntt2::polymul(&a, &b, MOD)
}

const MOD: usize = 100_003;
...
fn main() {
    ...
    for i in 0..n {
        nums[i] = vec![scan.token::<usize>(), 1];
    }

    // nums = {a, b, c} => f = (a, 1) * (b, 1) * (c, 1)
    //                  => f = (a * b * c, a * b + a * c + b * c, a + b + c, 1)
    // Therefore, use the polynomial multiplication using FFT
    // However, the naive computation is too slow, so we use the divide and conquer

    let ret = divide_and_conquer(&nums, 0, n - 1);
    let m = scan.token::<i64>();

    for _ in 0..m {
        let k = scan.token::<usize>();
        writeln!(out, "{}", ret[n - k]).unwrap();
    }
}
