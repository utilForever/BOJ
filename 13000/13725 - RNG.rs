...
mod ntt {
    ...
    pub use mod104857601::{INV, INV_P2, MOD, ROOTS};
...
fn poly_mul(mut a: Vec<usize>, mut b: Vec<usize>) -> Vec<usize> {
    let len_a = a.len();
    let len_b = b.len();
    ...
    let size = (len_a + len_b - 1).next_power_of_two();

    a.resize(size, 0);
    b.resize(size, 0);

    ntt::fft_opt(&mut a, false);
    ntt::fft_opt(&mut b, false);

    for i in 0..size {
        a[i] = (a[i] * b[i]) % MOD;
    }

    ntt::fft_opt(&mut a, true);
...
fn poly_inv(a: &[usize], m: usize) -> Vec<usize> {
    let n = a.len();
    let mut k = 1;
    let mut r = vec![inv(a[0])];

    while k < m {
        let mut s = a[..n.min(k << 1)].to_vec();
        s = poly_mul(s, r.clone());
...
fn poly_mod(a: Vec<usize>, mut b: Vec<usize>) -> Vec<usize> {
    let len_a = a.len();
    let len_b = b.len();
    ...
    while !r_a.is_empty() && *r_a.last().unwrap() == 0 {
        r_a.pop();
    }

    while !r_b.is_empty() && *r_b.last().unwrap() == 0 {
        r_b.pop();
    }

    r_a = poly_mul(r_a, poly_inv(&r_b, len_a - len_b + 1));
    r_a.resize(len_a - len_b + 1, 0);
    r_a.reverse();

    b = poly_mul(b, r_a);

    let mut ret = vec![0; len_b - 1];

    for i in 0..len_b - 1 {
        ret[i] = (a[i] + MOD - b[i]) % MOD;
    }

    while !ret.is_empty() && *ret.last().unwrap() == 0 {
        ret.pop();
    }
...
fn kitamasa(mut c: Vec<usize>, mut n: usize) -> Vec<usize> {
    let mut d = vec![1];
    let mut f = vec![0, 1];
    ...
    while n > 0 {
        if n & 1 == 1 {
            d = poly_mod(poly_mul(d, f.clone()), c.clone());
        }

        n /= 2;
        f = poly_mod(poly_mul(f.clone(), f), c.clone());
    }
