...
fn main() {
    ...
    let mut troops = vec![0; n];

    for i in 0..n {
        troops[i] = scan.token::<i64>();
    }
    ...
    for i in 0..n {
        let capacity = if i == 0 || i == n - 1 || troops[i] == -1 {
            INF
        } else {
            troops[i]
        };
        maximum_flow.add_edge(i, i + n, capacity);
    }

    for _ in 0..m {
        let (u, v) = (scan.token::<usize>() - 1, scan.token::<usize>() - 1);
        maximum_flow.add_edge(u + n, v, INF);
        maximum_flow.add_edge(v + n, u, INF);
    }

    let flow = maximum_flow.get_flow();

    writeln!(out, "{}", if flow <= k { "YES" } else { "NO" }).unwrap();
}
