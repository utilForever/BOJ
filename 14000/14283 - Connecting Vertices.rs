...
fn calculate_min_cut(
    ...
    let source = n;
    let sink = n + 1;
    let mut dinic = Dinic::new(n + 2, source, sink);

    for &edge in edges.iter() {
        dinic.add_edge(edge.0, edge.1, edge.2);
        dinic.add_edge(edge.1, edge.0, edge.2);
    }

    for &l in left {
        dinic.add_edge(source, l, INF);
    }

    for &r in right {
        dinic.add_edge(r, sink, INF);
    }
...
fn main() {
    ...
    let total = edges.iter().fold(0, |acc, &(_, _, c)| acc + c);
    let mut flow = INF;

    for &edge in edges.iter() {
        let cut = calculate_min_cut(&edges, &[s, edge.0], &[t, edge.1], n);
        flow = flow.min(cut - edge.2);

        let cut = calculate_min_cut(&edges, &[s, edge.1], &[t, edge.0], n);
        flow = flow.min(cut - edge.2);
    }
