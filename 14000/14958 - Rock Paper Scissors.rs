...
fn main() {
    ...
    let rps_machine = scan.token::<String>().chars().collect::<Vec<_>>();
    let mut rps_player = scan.token::<String>().chars().collect::<Vec<_>>();

    rps_player.reverse();

    let mut machine = vec![0; n];
    let mut player = vec![0; m];
    let mut result = vec![0; n + m - 1];

    for i in 0..n {
        machine[i] = if rps_machine[i] == 'R' { 1 } else { 0 };
    }

    for i in 0..m {
        player[i] = if rps_player[i] == 'P' { 1 } else { 0 };
    }

    let ret = ntt::polymul(&machine, &player);
    ...
    for i in 0..n {
        machine[i] = if rps_machine[i] == 'P' { 1 } else { 0 };
    }

    for i in 0..m {
        player[i] = if rps_player[i] == 'S' { 1 } else { 0 };
    }
    ...
    for i in 0..n {
        machine[i] = if rps_machine[i] == 'S' { 1 } else { 0 };
    }

    for i in 0..m {
        player[i] = if rps_player[i] == 'R' { 1 } else { 0 };
    }
