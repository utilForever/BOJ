...
fn main() {
    ...
    let mut intersections = vec![(0, 0, false); m];

    for i in 0..m {
        let (mut a, mut b) = (scan.token::<usize>() - 1, scan.token::<usize>() - 1);

        if a > b {
            std::mem::swap(&mut a, &mut b);
        }

        intersections[i] = (a, b, false);
    }

    for i in 0..m {
        let status = scan.token::<i64>();

        if status == 1 {
            intersections[i].2 = true;
            odc.add_edge(intersections[i].0, intersections[i].1);
        }
    }
        ...
        match command {
            1 => {
                ...
                if intersections[i].2 {
                    intersections[i].2 = false;
                    odc.remove_edge(intersections[i].0, intersections[i].1);
                } else {
                    intersections[i].2 = true;
                    odc.add_edge(intersections[i].0, intersections[i].1);
                }
