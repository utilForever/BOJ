...
fn main() {
    ...
    for i in 0..convex_hull.len() {
        if p3 == i || p3 == (i + 1) % convex_hull.len() {
            p3 = (i + 2) % convex_hull.len();
        }
        ...
        while cross(p1, p2, convex_hull[p3]).abs()
            < cross(p1, p2, convex_hull[(p3 + 1) % convex_hull.len()]).abs()
        {
            p3 = (p3 + 1) % convex_hull.len();
        }

        ret = ret.min(cross(p1, p2, convex_hull[p3]).abs() as f64 / dist_p1p2);
    }
