...
fn main() {
    ...
    let mut fibonacci = vec![String::new(); 491];
    fibonacci[1] = "1".to_string();
    fibonacci[2] = "1".to_string();

    for i in 3..=490 {
        fibonacci[i] = add(fibonacci[i - 1].clone(), fibonacci[i - 2].clone());
    }
    ...
    loop {
        ...
        writeln!(out, "Hour {n}: {} cow(s) affected", fibonacci[n as usize]).unwrap();
    }
}
