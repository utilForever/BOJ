...
fn main() {
    ...
    let mut mcmf = MCMF::new(n + 6, n + 4, n + 5);
    mcmf.add_edge(n + 4, n, 1, -1_000_000);
    mcmf.add_edge(n + 4, n, a - 1, 0);
    mcmf.add_edge(n + 4, n + 1, 1, -1_000_000);
    mcmf.add_edge(n + 4, n + 1, b - 1, 0);
    mcmf.add_edge(n + 4, n + 2, 1, -1_000_000);
    mcmf.add_edge(n + 4, n + 2, c - 1, 0);
    mcmf.add_edge(n + 4, n + 3, 1_000_000, 0);
    ...
    for i in 0..n {
        ...
        mcmf.add_edge(n, i, 1, -a);
        mcmf.add_edge(n + 1, i, 1, -b);
        mcmf.add_edge(n + 2, i, 1, -c);
        mcmf.add_edge(n + 3, i, 1, 0);
        mcmf.add_edge(i, n + 5, 1, 0);
    }

    let (_, mut cost) = mcmf.get_flow();
    cost *= -1;
    cost -= 3_000_000;
