...
fn calculate_color_transfer(points: &Vec<Point>, idxes: &Vec<usize>) -> f128 {
    if idxes.is_empty() {
        return f128::from(0.0);
    }

    let mut sum_r = f128::from(0.0);
    let mut sum_g = f128::from(0.0);
    let mut sum_b = f128::from(0.0);

    for &idx in idxes.iter() {
        sum_r += f128::from(points[idx].r);
        sum_g += f128::from(points[idx].g);
        sum_b += f128::from(points[idx].b);
    }

    let len = f128::from(idxes.len());
    let center_r = sum_r / len;
    let center_g = sum_g / len;
    let center_b = sum_b / len;

    let mut ret = f128::from(0.0);

    for &idx in idxes.iter() {
        let p = &points[idx];
        ret += (f128::from(p.r) - center_r).powi(2)
            + (f128::from(p.g) - center_g).powi(2)
            + (f128::from(p.b) - center_b).powi(2);
    }

    ret
}

fn kmeans_two(points: &Vec<Point>, i: usize, j: usize, max_iter: usize) -> Option<f128> {
    let n = points.len();
    let mut centroid1 = points[i];
    let mut centroid2 = points[j];
    let mut label_assigned = vec![0; n];

    for _ in 0..max_iter {
        let mut is_changed = false;

        for idx in 0..n {
            let dist1 = points[idx].dist2(&centroid1);
            let dist2 = points[idx].dist2(&centroid2);
            let label_new = if dist1 <= dist2 { 0 } else { 1 };

            if label_assigned[idx] != label_new {
                label_assigned[idx] = label_new;
                is_changed = true;
            }
        }
        ...
        for (idx, &label) in label_assigned.iter().enumerate() {
            let p = points[idx];
        ...
        let cnt0 = cnt[0] as f64;
        let cnt1 = cnt[1] as f64;

        centroid1 = Point::new(sum[0][0] / cnt0, sum[0][1] / cnt0, sum[0][2] / cnt0);
        centroid2 = Point::new(sum[1][0] / cnt1, sum[1][1] / cnt1, sum[1][2] / cnt1);
    ...
    let mut idx2 = Vec::with_capacity(n);
    ...
    Some(calculate_color_transfer(points, &idx1) + calculate_color_transfer(points, &idx2))
}
...
fn main() {
    ...
    let mut ret = calculate_color_transfer(&points, &idxes);

    if k == 2 {
        for i in 0..n {
            for j in i + 1..n {
                if let Some(cost) = kmeans_two(&points, i, j, 8) {
                    if cost < ret {
                        ret = cost;
                    }
                }
    ...
    writeln!(out, "{:.6}", f64::from(ret)).unwrap();
}
