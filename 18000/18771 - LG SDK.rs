...
fn main() {
    ...
    for _ in 0..t {
        ...
        let mut interests = vec![0; n];

        for i in 0..n {
            interests[i] = scan.token::<i64>();
        }
        ...
        for i in 0..n {
            if interests[i] > 0 {
                maximum_flow.add_edge(source, i, interests[i]);
                sum += interests[i];
            } else {
                maximum_flow.add_edge(i, sink, -interests[i]);
            }
        }
