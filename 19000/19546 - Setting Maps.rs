...
fn main() {
    ...
    let idx_in = |u: usize, v: usize| -> usize { (u * n + (v - 1)) * 2 };
    let idx_out = |u: usize, v: usize| -> usize { (u * n + (v - 1)) * 2 + 1 };

    let source = n * k * 2;
    let sink = n * k * 2 + 1;
    let mut maximum_flow = Dinic::new(n * k * 2 + 2, source, sink);

    for i in 0..k {
        for j in 1..=n {
            maximum_flow.add_edge(idx_in(i, j), idx_out(i, j), costs[j]);
        }
    }

    for &(u, v) in edges.iter() {
        for i in 0..k {
            maximum_flow.add_edge(idx_out(i, u), idx_in(i, v), INF);
        }
    }
    ...
    if k >= 2 {
        for i in 0..k - 1 {
            for j in 1..=n {
                maximum_flow.add_edge(idx_in(i, j), idx_out(i + 1, j), INF);
            }
        }
    }

    for i in 0..k {
        maximum_flow.add_edge(source, idx_in(i, s), INF);
        maximum_flow.add_edge(idx_out(i, e), sink, INF);
    }
