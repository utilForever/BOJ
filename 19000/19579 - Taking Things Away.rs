...
fn main() {
    ...
    for i in 0..n {
        if changes[i] > 0 {
            maximum_flow.add_edge(source, i, changes[i]);
        } else if changes[i] < 0 {
            maximum_flow.add_edge(i, sink, -changes[i]);
        }
    }

    for _ in 0..m {
        let (s, e) = (scan.token::<usize>() - 1, scan.token::<usize>() - 1);
        maximum_flow.add_edge(s, e, INF);
    }
    ...
    while let Some(node) = queue.pop_front() {
        for &next in maximum_flow.graph[node].iter() {
            if next.capacity > 0 && !visited[next.to] {
