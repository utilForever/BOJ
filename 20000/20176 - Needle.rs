...
fn main() {
    ...
    let mut line1 = vec![0; 60001];
    let mut line2 = vec![0; 60001];
    let mut line3 = vec![0; 60001];

    let n = scan.token::<usize>();

    for _ in 0..n {
        let x = scan.token::<i64>();
        line1[(x + 30000) as usize] = 1;
    }
    ...
    for _ in 0..k {
        let x = scan.token::<i64>();
        line3[(x + 30000) as usize] = 1;
    }

    let result = ntt::polymul(&line1, &line3);
    let mut ret = 0;

    for i in 0..=60000 {
        ret += result[i * 2] * line2[i];
    }
