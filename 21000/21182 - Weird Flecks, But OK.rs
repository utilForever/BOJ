...
fn main() {
    ...
    let mut points1 = vec![Point::default(); n];
    let mut points2 = vec![Point::default(); n];
    let mut points3 = vec![Point::default(); n];
    ...
    for i in 0..n {
        ...
        points1[i] = Point::new(x, y);
        points2[i] = Point::new(y, z);
        points3[i] = Point::new(x, z);
    }

    let circle1 = Circle::welzl(&points1);
    let circle2 = Circle::welzl(&points2);
    let circle3 = Circle::welzl(&points3);
    let ret = circle1.radius.min(circle2.radius).min(circle3.radius);

    writeln!(out, "{:.6}", ret * 2.0).unwrap();
}
