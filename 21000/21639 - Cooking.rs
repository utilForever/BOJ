...
fn main() {
    ...
    let mut times = vec![vec![0; n]; n];
    let mut dishes_total = 0;

    for i in 0..n {
        dishes[i] = scan.token::<usize>();
        dishes_total += dishes[i];
    }

    for i in 0..n {
        for j in 0..n {
            times[i][j] = scan.token::<i64>();
        }
    }
    ...
    let mut vertices = Vec::with_capacity(dishes_total);
    let mut idx = 0;
    ...
    for i in 0..vertices.len() - 1 {
        for j in i + 1..vertices.len() {
            let weight = 100 - times[vertices[i].1][vertices[j].1];
            edges.push((vertices[i].0, vertices[j].0, weight));
        }
    }
    ...
    for (v1, &v2) in matching.iter().enumerate() {
        ...
        ret += times[vertices[v1].1][vertices[v2].1];
    }
