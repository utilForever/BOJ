...
fn main() {
    ...
    let mut maximum_flow = MaximumFlow::new(n + m + 3, 0, n + m + 2);
    ...
    for i in 1..=m {
        maximum_flow.add_edge(n + i, n + m + 2, 1);
    }
    ...
    for i in 1..=n {
        ...
        for _ in 0..k {
            ...
            maximum_flow.add_edge(i, n + a, 1);
        }
    }

    maximum_flow.get_flow();

    maximum_flow.add_edge(0, n + m + 1, 1);

    for i in 1..=m {
        maximum_flow.add_edge(n + m + 1, n + i, 1);
    }

    writeln!(
        out,
        "{}",
        if maximum_flow.get_flow() > 0 as i64 {
            "Alice"
        } else {
            "Bob"
        }
