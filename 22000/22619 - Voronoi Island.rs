...
fn main() {
    ...
    loop {
        ...
        for _ in 0..n {
            let (x, y) = (scan.token::<i64>() as f64, scan.token::<i64>() as f64);
            boundaries.push(Point { x, y });
        }
        ...
        let diagram = VoronoiDiagram::with_bounding_polygon(
            points.clone(),
            &Polygon::from_points(boundaries),
        )
        .unwrap();
        ...
        for cell in cells {
            ...
            ret[mark_to_add] += calculate_area(&cell.points);
        }

        for area in ret {
            writeln!(out, "{:.4}", area).unwrap();
        }
    }
}
