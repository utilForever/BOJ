...
fn main() {
    ...
    for _ in 0..n {
        let (x, k) = (scan.token::<i64>(), scan.token::<usize>());
        let mut visited = vec![false; m + 1];
        let mut required = Vec::with_capacity(k);
        ...
        orders.push((x, required));
    }

    let mut prices = vec![0; m + 1];

    for i in 1..=m {
        prices[i] = scan.token::<i64>();
    }

    let mut discounts = Vec::with_capacity(p);
    ...
    for _ in 0..p {
        ...
        discounts.push((a, b, d));
    }

    let source = 0;
    let sink = n + m + p + 1;
    let mut maximum_flow = Dinic::new(n + m + p + 2, source, sink);
    let mut total = 0;

    for (idx, &(x, ref required)) in orders.iter().enumerate() {
        total += x;
        maximum_flow.add_edge(source, idx + 1, x);

        for &t in required.iter() {
            maximum_flow.add_edge(idx + 1, n + t, INF);
        }
    }

    for i in 1..=m {
        maximum_flow.add_edge(n + i, sink, prices[i]);
    }

    for (idx, &(a, b, d)) in discounts.iter().enumerate() {
        total += prices[a] + prices[b] - d;

        maximum_flow.add_edge(source, n + m + 1 + idx, prices[a] + prices[b] - d);
        maximum_flow.add_edge(n + m + 1 + idx, n + a, INF);
        maximum_flow.add_edge(n + m + 1 + idx, n + b, INF);
    }

    let flow = maximum_flow.get_flow();

    writeln!(out, "{}", total - flow).unwrap();
}
