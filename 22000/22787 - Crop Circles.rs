        ...
        for i in 0..self.n {
            ...
            for j in 0..self.n {
                ...
                if sign(temp.0) <= 0 && sign(temp.1) <= 0 {
                    self.segments.push((2.0 * PI + temp.0, 2.0 * PI + temp.1));
                } else if sign(temp.0) < 0 {
                    self.segments.push((2.0 * PI + temp.0, 2.0 * PI));
...
fn main() {
    ...
    loop {
        ...
        let ret = circle_union.union(n);

        if ret < 10.0 {
            writeln!(out, "Data set #{t}: Total area = {:.4} square meters.", ret).unwrap();
        } else {
            writeln!(
                out,
                "Data set #{t}:  Total area = {:.4} square meters.",
                ret
            )
            .unwrap();
        }
