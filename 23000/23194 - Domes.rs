...
fn main() {
    ...
    let mut domes = vec![Point::new(0.0, 0.0); n + 1];

    for i in 1..=n {
        domes[i] = Point::new(scan.token::<f64>(), scan.token::<f64>());
    }
    ...
    let mut lines = Vec::with_capacity(n * (n - 1) / 2 + 4);

    lines.push(Line::new(Point::new(0.0, 0.0), Point::new(dx, 0.0)));
    lines.push(Line::new(Point::new(dx, 0.0), Point::new(dx, dy)));
    lines.push(Line::new(Point::new(dx, dy), Point::new(0.0, dy)));
    lines.push(Line::new(Point::new(0.0, dy), Point::new(0.0, 0.0)));

    for i in 0..n {
        for j in i + 1..n {
            lines.push(Line::new(domes[order[j]], domes[order[i]]));
        }
    }
