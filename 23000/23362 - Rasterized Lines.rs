...
fn main() {
    ...
    for _ in 0..t {
        ...
        let mut factors_new = HashMap::new();

        for val in factors.iter() {
            let count = factors_new.entry(val).or_insert(0);
            *count += 1;
        }
        ...
        for factor in factors_new.iter() {
            let divisors_new = divisors.clone();

            for i in 0..*factor.1 {
                for divisor in divisors_new.iter() {
                    divisors.push(divisor * (**factor.0 as usize).pow(i as u32 + 1));
                }
            }
        }
