...
fn main() {
    ...
    let mut performances = vec![vec![0; m]; n];
    let mut performances_max = vec![0; n];
    let mut base = 0;

    for i in 0..n {
        let mut performance_max = 0;

        for j in 0..m {
            performances[i][j] = scan.token::<i64>();
            performance_max = performance_max.max(performances[i][j]);
        }

        performances_max[i] = performance_max;
        base += performance_max;
    }

    let mut friends = vec![(0, 0); k];

    for i in 0..k {
        friends[i] = (scan.token::<usize>() - 1, scan.token::<usize>() - 1);
    }

    let source = 0;
    let sink = 1;
    let mut maximum_flow = Dinic::new(n * (m - 1) + 2, source, sink);
    let mut sum_constant = 0;

    let idx = |i: usize, j: usize, m: usize| 2 + i * (m - 1) + j - 1;
    ...
    for i in 0..n {
        if m >= 2 {
            ...
            for j in 1..m {
                let delta = performances[i][j - 1] - performances[i][j];

                if delta > 0 {
                    maximum_flow.add_edge(idx(i, j, m), sink, delta);
                } else if delta < 0 {
                    maximum_flow.add_edge(source, idx(i, j, m), -delta);
                    sum += -delta;
                }
            }

            sum_constant += sum - performances_max[i] + performances[i][0];
        }
    }
    ...
    if m >= 2 && c > 0 {
        for (a, b) in friends {
            for i in 1..m {
                ...
                maximum_flow.add_edge(u, v, c);
                maximum_flow.add_edge(v, u, c);
            }
        }
    }
    ...
    writeln!(out, "{}", base + sum_constant - flow).unwrap();
}
