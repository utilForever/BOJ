...
fn main() {
    ...
    let source = n + 3 * 256;
    let sink = n + 3 * 256 + 1;
    let mut maximum_flow = Dinic::new(n + 3 * 256 + 2, source, sink);

    for i in 0..256 {
        maximum_flow.add_edge(i, sink, ca);
        maximum_flow.add_edge(i + 256, sink, ck);
        maximum_flow.add_edge(i + 512, sink, ct);
    }

    for (idx, &(a, k, t)) in insects.iter().enumerate() {
        maximum_flow.add_edge(source, 3 * 256 + idx, p);
        maximum_flow.add_edge(3 * 256 + idx, a, INF);
        maximum_flow.add_edge(3 * 256 + idx, k + 256, INF);
        maximum_flow.add_edge(3 * 256 + idx, t + 512, INF);
    }

    let flow = maximum_flow.get_flow();

    writeln!(out, "{}", n as i64 * p - flow).unwrap();
}
