...
fn main() {
    ...
    let source = 2 * n;
    let sink = 2 * n + 1;
    let mut maximum_flow = Dinic::new(2 * n + 2, source, sink);

    maximum_flow.add_edge(source, a, INF);
    maximum_flow.add_edge(source, b + n, INF);
    maximum_flow.add_edge(a + n, sink, INF);
    maximum_flow.add_edge(b, sink, INF);

    for &(u, v, l) in edges.iter() {
        maximum_flow.add_edge(u, v + n, l);
        maximum_flow.add_edge(v + n, u, l);
        maximum_flow.add_edge(v, u + n, l);
        maximum_flow.add_edge(u + n, v, l);
    }

    let flow = maximum_flow.get_flow();

    maximum_flow.process_bfs();

    let mut ret = vec![' '; n];

    for i in 0..n {
        let r1 = maximum_flow.check[i] != -1;
        let r2 = maximum_flow.check[i + n] != -1;
