...
fn main() {
    ...
    if n == 1 {
        ...
        for point in [(0.0, 0.0), (0.0, h), (w, 0.0), (w, h)].iter() {
            ret = ret.max((points[0].x - point.0).hypot(points[0].y - point.1));
        }
    ...
    let diagram =
        VoronoiDiagram::new(&Point { x: 0.0, y: 0.0 }, &Point { x: w, y: h }, &points).unwrap();
    ...
    for cell in cells {
        for i in 0..points.len() {
            if is_inside(&points[i], &cell.points) {
                for p in cell.points.iter() {
                    ret = ret.max((p.x - points[i].x).hypot(p.y - points[i].y));
                }
