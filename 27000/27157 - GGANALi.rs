...
fn set_property(
    ...
    match name_property.as_str() {
        ...
        "SENSITIVE" => {
            let v = scan.token::<i64>();
            actors.get_mut(&name_actor).unwrap().sensitive = v;
        }
        "CLIP_CHILD" => {
            let v = scan.token::<i64>();
            actors.get_mut(&name_actor).unwrap().clip_child = v;
        }
...
fn get_property(
    ...
    match name_property.as_str() {
        ...
        "SENSITIVE" => {
            writeln!(out, "{}", actors[&name_actor].sensitive).unwrap();
        }
        "CLIP_CHILD" => {
            writeln!(out, "{}", actors[&name_actor].clip_child).unwrap();
        }
        _ => panic!("Unknown property"),
    }
}
...
fn touch(
    ...
    if actors[&name_node].sensitive == 1 {
        if x <= x_touch
            && x_touch < x + actors[&name_node].size.0
            && x1 <= x_touch
            && x_touch < x2
            && y <= y_touch
            && y_touch < y + actors[&name_node].size.1
            && y1 <= y_touch
            && y_touch < y2
        {
            ret = name_node.clone();
    ...
    if actors[&name_node].clip_child == 1 {
        x1 = x1.max(x);
        y1 = y1.max(y);
        x2 = x2.min(x + actors[&name_node].size.0);
        y2 = y2.min(y + actors[&name_node].size.1);
    }
