...
fn calculate(
    ...
    for i in 0..m {
        let capacity = if Some(i) == chosen { INF } else { 1 };
        maximum_flow.add_edge(source, 1 + i, capacity);
    }

    for &(u, v) in edges.iter() {
        maximum_flow.add_edge(1 + u, m + 1 + v, INF);
    }

    for i in 0..n {
        maximum_flow.add_edge(m + 1 + i, sink, 1);
    }

    let _ = maximum_flow.get_flow();
    let visited = maximum_flow.reachable_from_source();
...
fn main() {
    ...
    let (chosen, _) = calculate(&edges, None, n, m);

    if !chosen.is_empty() {
        writeln!(out, "{}", chosen.len()).unwrap();
    ...
    for u in 0..m {
        let (chosen, killed) = calculate(&edges, Some(u), n, m);
        let score = chosen.len() as i64 - killed as i64;
    ...
    writeln!(out, "{}", best_set.len()).unwrap();
