...
fn main() {
    ...
    let mut spots = vec![0; n + 1];

    for i in 1..=n {
        spots[i] = scan.token::<i64>();
    }

    let mut spots_rev = vec![0; n + 1];

    for i in 1..=n {
        spots_rev[i] = -spots[n + 1 - i];
    }
    ...
    for i in 2..=n {
        tree_left.update(i, 2 * spots_rev[i] - spots_rev[i - 1]);
        tree_right.update(i, spots_rev[i] - 2 * spots_rev[i - 1]);
    }

    let mut dists_rev = vec![0; n + 1];

    for i in 1..=n {
        if i == 1 || i == n {
            dists_rev[i] = spots_rev[n] - spots_rev[1];
            continue;
        }
        ...
        let mut left = i - 1;
        let mut right = i + 1;
        let mut direction = if spots_rev[i] - spots_rev[left] >= spots_rev[right] - spots_rev[i] {
            1
        } else {
            -1
        };
        ...
        loop {
            if direction == 1 {
                let pos = match tree_right.query_first(idx + 2, -spots_rev[left]) {
                    Some(pos) => pos - 1,
                    None => n,
                };

                ret += spots_rev[pos] - spots_rev[idx];
                idx = pos;
                right = idx + 1;
            } else {
                let pos = match tree_left.query_last(idx - 1, spots_rev[right]) {
                    Some(pos) => pos,
                    None => 1,
                };

                ret += spots_rev[idx] - spots_rev[pos];
                idx = pos;
                left = idx - 1;
            }
        ...
        ret += if left >= 1 {
            spots_rev[idx] - spots_rev[1]
        } else {
            spots_rev[n] - spots_rev[idx]
        };
    ...
    for _ in 0..q {
        ...
        let idx = if left == 1 {
            ...
            n
        } else if s - spots[left - 1] <= spots[left] - s {
            left - 1
        } else {
            left
        };

        writeln!(out, "{}", (spots[idx] - s).abs() + dists[idx]).unwrap();
    }
}
