...
// Reference: https://anz1217.tistory.com/161
// Reference: ROTE, Günter. Crossing the bridge at night. Bulletin of the EATCS, 2002, 78: 241-.
fn main() {
    ...
    // Based on the paper "Crossing the bridge at night", but it's little bit different.
    // We can't move two ships with same size, so we need to consider the case.
    //
    // Strategy:
    // 1. Calculate an initial total cost (`cost_total`) based on a naive strategy that moves all ships individually
    //    and accounts for additional constraints.
    // 2. Identify potential cost savings by optimally pairing ships of different sizes.
    // 3. Use a maximum weight matching algorithm to find the optimal pairings that maximize cost savings.
    // 4. Adjust the total cost by subtracting the total cost savings (`cost_save`) from the initial total cost.
    //
    // Calculating `cost_total`:
    // - Sum of all ship sizes: Each ship must cross at least once.
    // - `(n - cnt_ship_first - 2) * sizes[0]`: Additional trips where the smallest ship returns to assist others.
    // - `2 * (cnt_ship_first - 1) * sizes[pos_ship_second]`: Extra cost due to the constraint that the smallest
    //   ships cannot move together; we use the next smallest ship to assist.
    //
    // The idea is to model the problem as a graph where:
    // - Nodes represent ships.
    // - Edges represent possible pairings between ships of different sizes.
    // - Edge weights represent the amount of cost saved by pairing those two ships.
    //
    // By finding the maximum weight matching, we maximize the total cost savings, leading to the minimal total cost.
    let cnt_ship_first = sizes.iter().filter(|&&x| x == sizes[0]).count();
    let pos_ship_second = sizes.iter().position(|&x| x > sizes[0]).unwrap();
    let cost_total = sizes[cnt_ship_first - 1..].iter().sum::<i64>()
        + (n as i64 - cnt_ship_first as i64 - 2) * sizes[0]
        + 2 * (cnt_ship_first - 1) as i64 * sizes[pos_ship_second];
    ...
    for i in 0..n - 1 {
        for j in i + 1..n {
            // We can't move two ships with same size.
            if sizes[i] == sizes[j] {
                continue;
            }

            // Calculate the potential cost savings when pairing ships i and j.
            //
            // Cost if moved separately:
            // - The smallest ship (sizes[0]) moves with each of sizes[i] and sizes[j] separately.
            // - Total cost: sizes[0] + sizes[i] + sizes[0] + sizes[j] = 2 * sizes[0] + sizes[i] + sizes[j].
            //
            // Cost if moved together:
            // - We use sizes[pos_ship_second] (the next smallest ship) to assist in moving sizes[i] and sizes[j] together.
            // - Total cost: sizes[pos_ship_second] + max(sizes[i], sizes[j]) + sizes[pos_ship_second].
            //
            // The cost saving (weight) is the difference between moving separately and moving together.
            //
            // Simplifying, we calculate:
            // weight = (sizes[0] + sizes[i]) - 2 * sizes[pos_ship_second];
            let weight = sizes[0] + sizes[i] - 2 * sizes[pos_ship_second];
    ...
    // Process maximal weight matching on general graph.
    let matching = Matching::new(edges).solve();
    let mut cost_save = 0;

    // Selected vertices are maximal values to save cost.
    // => The minimum cost of the set that satisfies the condition.
    for (v1, &v2) in matching.iter().enumerate() {
        if v2 == SENTINEL || v1 >= v2 {
            continue;
        }

        cost_save += weights.get(&(v1, v2)).unwrap();
    }

    writeln!(out, "{}", cost_total - cost_save).unwrap();
}
