...
fn main() {
    ...
    let mut intervals = vec![(0, 0); q];

    for i in 0..q {
        intervals[i] = (scan.token::<usize>() - 1, scan.token::<usize>() - 1);
    }

    let mut nums = vec![0; 2 * q - 1];

    for i in 0..2 * q - 1 {
        nums[i] = scan.token::<i64>();
    }

    let mut prefix_sum = vec![0; 2 * q];

    for i in 1..2 * q {
        prefix_sum[i] = prefix_sum[i - 1] + nums[i - 1];
    }

    let cost_total = intervals
        .iter()
        .map(|&(a, b)| prefix_sum[b] - prefix_sum[a])
        .sum::<i64>();
    let mut weights = vec![vec![0; q]; q];
    ...
    for i in 0..q {
        for j in 0..q {
            ...
            // => We can split (L_i, R_j) into (L_i, L_j), (L_j, R_i) and (R_i, R_j)
            //    if the conditions L_i < L_j && L_j < R_i && R_i < R_j are true
            if intervals[i].0 < intervals[j].0
                && intervals[j].0 < intervals[i].1
                && intervals[i].1 < intervals[j].1
            {
                // The weight is the sum of costs for overlapping interval
                let weight = prefix_sum[intervals[i].1] - prefix_sum[intervals[j].0];
                weights[i][j] = weight;
                weights[j][i] = weight;
    ...
    let matching = Matching::new(edges.clone()).solve();
    let mut cost_save = 0;
