...
fn main() {
    ...
    let mut prices_cake = vec![0; c];
    let mut prices_ingredient = vec![0; g];
    let mut prices_tool = vec![0; t];

    for i in 0..c {
        prices_cake[i] = scan.token::<i64>();
    }

    for i in 0..g {
        prices_ingredient[i] = scan.token::<i64>();
    }

    for i in 0..t {
        prices_tool[i] = scan.token::<i64>();
    }

    let mut amounts = vec![vec![0; g]; c];

    for i in 0..c {
        for j in 0..g {
            amounts[i][j] = scan.token::<i64>();
        }
    }

    let mut required = vec![Vec::new(); c];

    for i in 0..c {
        let k = scan.token::<usize>();

        for _ in 0..k {
            required[i].push(scan.token::<usize>() - 1);
        }
    }

    let mut profits = vec![0; c];

    for i in 0..c {
        let mut cost = 0;

        for j in 0..g {
            cost += amounts[i][j] * prices_ingredient[j];
        }

        profits[i] = prices_cake[i] - cost;
    }

    let source = 0;
    let sink = c + t + 1;
    let mut maximum_flow = Dinic::new(c + t + 2, source, sink);

    for i in 0..c {
        if profits[i] > 0 {
            maximum_flow.add_edge(source, i + 1, profits[i]);
        } else {
            maximum_flow.add_edge(i + 1, sink, -profits[i]);
        }
    }

    for i in 0..t {
        if prices_tool[i] > 0 {
            maximum_flow.add_edge(c + i + 1, sink, prices_tool[i]);
        }
    }

    for i in 0..c {
        for &tool in required[i].iter() {
            maximum_flow.add_edge(1 + i, c + tool + 1, INF);
        }
    }
    ...
    writeln!(
        out,
        "{}",
        profits.iter().filter(|&&x| x > 0).sum::<i64>() - flow
    )
    .unwrap();
}
