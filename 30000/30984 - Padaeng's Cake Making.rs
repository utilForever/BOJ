...
// Reference: https://blog.naver.com/dylan0301/221862713422
fn main() {
    ...
    let mut ret1 = pow(factorial[k], n as i64);

    for i in 0..=n - 1 {
        ret1 = ret1 * factorial[i] % MOD;
    }

    let mut ret2 = 1;

    for i in k..=n + k - 1 {
        ret2 = ret2 * factorial[i] % MOD;
    }

    writeln!(out, "{}", ret1 * pow(ret2, MOD - 2) % MOD).unwrap();
}
