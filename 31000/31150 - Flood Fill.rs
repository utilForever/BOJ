use io::Write;
use std::{collections::VecDeque, io, str};

pub struct UnsafeScanner<R> {
    reader: R,
    buf_str: Vec<u8>,
    buf_iter: str::SplitAsciiWhitespace<'static>,
}

impl<R: io::BufRead> UnsafeScanner<R> {
    pub fn new(reader: R) -> Self {
        Self {
            reader,
            buf_str: vec![],
            buf_iter: "".split_ascii_whitespace(),
        }
    }

    pub fn token<T: str::FromStr>(&mut self) -> T {
        loop {
            if let Some(token) = self.buf_iter.next() {
                return token.parse().ok().expect("Failed parse");
            }
            self.buf_str.clear();
            self.reader
                .read_until(b'\n', &mut self.buf_str)
                .expect("Failed read");
            self.buf_iter = unsafe {
                let slice = str::from_utf8_unchecked(&self.buf_str);
                std::mem::transmute(slice.split_ascii_whitespace())
            }
        }
    }

    pub fn all(&mut self) -> String {
        let mut input = String::new();
        self.reader.read_to_string(&mut input).expect("Failed read");
        input
    }

    pub fn line(&mut self) -> String {
        let mut input = String::new();
        self.reader.read_line(&mut input).expect("Failed read");
        input
    }
}

#[derive(Clone, Copy)]
struct Edge {
    to: usize,
    rev: usize,
    capacity: i64,
}

impl Edge {
    fn new(to: usize, rev: usize, capacity: i64) -> Self {
        Self { to, rev, capacity }
    }
}

struct Dinic {
    graph: Vec<Vec<Edge>>,
    source: usize,
    sink: usize,
    check: Vec<i64>,
    done: Vec<usize>,
}

impl Dinic {
    fn new(n: usize, source: usize, sink: usize) -> Self {
        Self {
            graph: vec![Vec::new(); n],
            source,
            sink,
            check: vec![-1; n],
            done: vec![0; n],
        }
    }

    fn add_edge(&mut self, u: usize, v: usize, capacity: i64) {
        let orig = self.graph[v].len();
        let dest = self.graph[u].len();

        self.graph[u].push(Edge::new(v, orig, capacity));
        self.graph[v].push(Edge::new(u, dest, 0));
    }

    fn process_bfs(&mut self) -> bool {
        self.check.fill(-1);

        let mut queue = VecDeque::new();

        queue.push_back(self.source);
        self.check[self.source] = 0;

        while let Some(u) = queue.pop_front() {
            for &e in self.graph[u].iter() {
                if e.capacity > 0 && self.check[e.to] < 0 {
                    queue.push_back(e.to);
                    self.check[e.to] = self.check[u] + 1;
                }
            }
        }

        self.check[self.sink] >= 0
    }

    fn process_dfs(&mut self, idx: usize, flow: i64) -> i64 {
        if idx == self.sink {
            return flow;
        }

        let n = self.graph[idx].len();

        while self.done[idx] < n {
            let edge = self.graph[idx][self.done[idx]];

            if edge.capacity > 0 && self.check[edge.to] == self.check[idx] + 1 {
                let flow_current = self.process_dfs(edge.to, flow.min(edge.capacity));

                if flow_current > 0 {
                    self.graph[idx][self.done[idx]].capacity -= flow_current;
                    self.graph[edge.to][edge.rev].capacity += flow_current;

                    return flow_current;
                }
            }

            self.done[idx] += 1;
        }

        0
    }

    fn get_flow(&mut self) -> i64 {
        let mut flow_total = 0;

        while self.process_bfs() {
            self.done.fill(0);

            loop {
                let flow_current = self.process_dfs(self.source, i64::MAX);

                if flow_current == 0 {
                    break;
                }

                flow_total += flow_current;
            }
        }

        flow_total
    }
}

const INF: i64 = 1_000_000_000_000;
const DY: [isize; 4] = [-1, 1, 0, 0];
const DX: [isize; 4] = [0, 0, -1, 1];

fn main() {
    let (stdin, stdout) = (io::stdin(), io::stdout());
    let mut scan = UnsafeScanner::new(stdin.lock());
    let mut out = io::BufWriter::new(stdout.lock());

    let (n, m) = (scan.token::<usize>(), scan.token::<usize>());
    let mut a = vec![vec![0; m]; n];
    let mut b = vec![vec![0; m]; n];

    for i in 0..n {
        let s = scan.token::<String>();

        for (j, c) in s.chars().enumerate() {
            a[i][j] = c as u8 - b'0';
        }
    }

    for i in 0..n {
        let s = scan.token::<String>();

        for (j, c) in s.chars().enumerate() {
            b[i][j] = c as u8 - b'0';
        }
    }

    let mut component_id = vec![vec![usize::MAX; m]; n];
    let mut component_color = Vec::new();
    let mut component_equal = Vec::new();
    let mut component_different = Vec::new();
    let mut id = 0;

    for i in 0..n {
        for j in 0..m {
            if component_id[i][j] != usize::MAX {
                continue;
            }

            let color = a[i][j];
            let mut queue = VecDeque::new();

            component_id[i][j] = id;
            queue.push_back((i, j));

            let mut cnt_equal = 0;
            let mut cnt_different = 0;

            while let Some((y_curr, x_curr)) = queue.pop_front() {
                if a[y_curr][x_curr] == b[y_curr][x_curr] {
                    cnt_equal += 1;
                } else {
                    cnt_different += 1;
                }

                for k in 0..4 {
                    let y_next = y_curr as isize + DY[k];
                    let x_next = x_curr as isize + DX[k];

                    if y_next < 0 || y_next >= n as isize || x_next < 0 || x_next >= m as isize {
                        continue;
                    }

                    let y_next = y_next as usize;
                    let x_next = x_next as usize;

                    if component_id[y_next][x_next] == usize::MAX && a[y_next][x_next] == color {
                        component_id[y_next][x_next] = id;
                        queue.push_back((y_next, x_next));
                    }
                }
            }

            component_color.push(color);
            component_equal.push(cnt_equal);
            component_different.push(cnt_different);
            id += 1;
        }
    }

    let source = id;
    let sink = id + 1;
    let mut maximum_flow = Dinic::new(id + 2, source, sink);

    for i in 0..id {
        if component_color[i] == 0 {
            maximum_flow.add_edge(source, i, component_different[i]);
            maximum_flow.add_edge(i, sink, component_equal[i]);
        } else {
            maximum_flow.add_edge(source, i, component_equal[i]);
            maximum_flow.add_edge(i, sink, component_different[i]);
        }
    }

    for i in 0..n {
        for j in 0..m {
            if i + 1 < n && component_id[i][j] != component_id[i + 1][j] {
                let (u, v) = if component_color[component_id[i][j]] == 0 {
                    (component_id[i][j], component_id[i + 1][j])
                } else {
                    (component_id[i + 1][j], component_id[i][j])
                };

                maximum_flow.add_edge(u, v, INF);
            }

            if j + 1 < m && component_id[i][j] != component_id[i][j + 1] {
                let (u, v) = if component_color[component_id[i][j]] == 0 {
                    (component_id[i][j], component_id[i][j + 1])
                } else {
                    (component_id[i][j + 1], component_id[i][j])
                };

                maximum_flow.add_edge(u, v, INF);
            }
        }
    }

    let flow = maximum_flow.get_flow();

    writeln!(out, "{flow}").unwrap();
}
