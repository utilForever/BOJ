...
fn main() {
    ...
    let mut mcmf = PushRelabel::new(n, 0, n - 1);
    ...
    for _ in 0..m {
        ...
        if a == 0 {
            mcmf.add_edge(u - 1, v - 1, b, c);
        } else {
            let calculate_cost = |x: i64| -> i64 { a * x * x + b * x };

            for i in 1..=c {
                mcmf.add_edge(u - 1, v - 1, calculate_cost(i) - calculate_cost(i - 1), 1);
            }
        }
    }

    let (flow, cost) = mcmf.get_min_cost_max_flow();
    writeln!(out, "{flow} {cost}").unwrap();
}
