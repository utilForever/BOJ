...
fn main() {
    ...
    let source = 2 * n * n;
    let sink = 2 * n * n + 1;
    let mut maximum_flow = Dinic::new(2 * n * n + 2, source, sink);
    ...
    for i in 0..n {
        for j in 0..n {
            ...
            if (i + j) % 2 == 0 {
                maximum_flow.add_edge(source, x, ((1 + a[i][j]) * b[i][j]).max(0));
                maximum_flow.add_edge(y, sink, ((1 - a[i][j]) * b[i][j]).max(0));
            } else {
                maximum_flow.add_edge(source, x, ((1 - a[i][j]) * b[i][j]).max(0));
                maximum_flow.add_edge(y, sink, ((1 + a[i][j]) * b[i][j]).max(0));
            }

            maximum_flow.add_edge_with_rev(x, y, a[i][j].abs() * b[i][j], INF);
        }
    }
    ...
    for i in 0..n {
        for j in 0..n {
            ...
            if i + 1 < n {
                ...
                maximum_flow.add_edge_with_rev(x1, x2, k, k);
                maximum_flow.add_edge_with_rev(y1, y2, k, k);
                maximum_flow.add_edge_with_rev(x1, y2, 3 * k, 3 * k);
                maximum_flow.add_edge_with_rev(y1, x2, 3 * k, 3 * k);
            }
    ...
    writeln!(out, "{}", 4 * k * (2 * n as i64 * (n as i64 - 1)) - flow).unwrap();
}
