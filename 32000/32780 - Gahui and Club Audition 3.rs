...
fn main() {
    ...
    let lv = scan.token::<usize>();
    let (cnt_normal, cnt_rev) = if lv <= 5 { (4, 0) } else { (8, 8) };
    let mut factorial = vec![1; 50_001];
    ...
    for i in 0..=lv.min(6) {
        let combinations = combination(&factorial, lv, i);
        let cnt_select_normal = pow(cnt_normal, (lv - i) as i64);
        let cnt_select_rev = pow(cnt_rev, i as i64);

        let total = ((combinations * cnt_select_rev) % MOD * cnt_select_normal) % MOD;
        ret = (ret + total) % MOD;
    }
