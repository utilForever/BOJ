...
fn dist_knight(dx: i64, dy: i64) -> i64 {
    let mut x = dx.abs();
    let mut y = dy.abs();
...
fn main() {
    ...
    for _ in 0..t {
        ...
        let mut maximum_flow = Dinic::new(10, 0, 9);

        for (i, (dx, dy)) in MOVES.iter().enumerate() {
            let rem = dist_knight(x - dx, y - dy);

            if rem + 1 == d {
                maximum_flow.add_edge(0, i + 1, 1);
            }

            maximum_flow.add_edge(i + 1, 9, 1);
        }
