...
fn calculate(yunee: &Vec<usize>, dalgu: &Vec<usize>, is_equal: bool) -> i64 {
    let (mut j, mut i, mut ret) = (0, 0, 0);

    while j < dalgu.len() && i < yunee.len() {
        if (is_equal && yunee[i] > dalgu[j]) || (!is_equal && yunee[i] < dalgu[j]) {
            j += 1;
            i += 1;
            ret += 1;
        } else if is_equal {
...
fn main() {
    ...
    let mut cards_yunee = [Vec::new(), Vec::new()];
    let mut cards_dalgu = [Vec::new(), Vec::new()];

    for _ in 0..n {
        let num = scan.token::<usize>();
        cards_yunee[num % 2].push(num);
    }

    for _ in 0..n {
        let num = scan.token::<usize>();
        cards_dalgu[num % 2].push(num);
    }

    for i in 0..2 {
        cards_yunee[i].sort_unstable();
        cards_dalgu[i].sort_unstable();
    }

    let source = 0;
    let sink = 5;
    let mut maximum_flow = Dinic::new(6, source, sink);

    for i in 0..2 {
        maximum_flow.add_edge(source, i + 1, cards_dalgu[i].len() as i64);
        maximum_flow.add_edge(i + 3, sink, cards_yunee[i].len() as i64);

        for j in 0..2 {
            maximum_flow.add_edge(
                i + 1,
                j + 3,
                calculate(&cards_yunee[j], &cards_dalgu[i], i == j),
            );
        }
    }
