...
fn move_blocks(board: &mut Vec<Vec<i64>>, direction: usize) -> i64 {
    let mut ret = 0;
    let n = board.len();
...
fn process_game(board: &mut Vec<Vec<i64>>, val_max: &mut i64, num_turn: u32) {
    if num_turn == 5 {
    ...
    for i in 0..4 {
        ...
        let ret = move_blocks(&mut board_new, i);

        if board_new != *board && *val_max < ret * 2_i64.pow(4 - num_turn) {
            process_game(&mut board_new, val_max, num_turn + 1);
        }
    }
}
...
fn main() {
    ...
    process_game(&mut board, &mut ret, 0);
