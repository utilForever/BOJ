...
impl LinkCutTree {
    ...
    unsafe fn _is_connect(&mut self, x: *mut Node, y: *mut Node) -> bool {
        self._get_root(x) == self._get_root(y)
    }

    unsafe fn _get_root(&mut self, mut node: *mut Node) -> *mut Node {
        // Make chain to root
        self.access(node);
    ...
    unsafe {
        ...
        for _ in 0..m {
            ...
            if command == 1 {
                ...
                tree.link(tree.nodes[u], tree.nodes[v]);
            } else if command == 2 {
                let v = scan.token::<usize>();
                tree.cut(tree.nodes[v]);
            } else {
                let (u, v) = (scan.token::<usize>(), scan.token::<usize>());
                let lca = tree.get_lca(tree.nodes[u], tree.nodes[v]);
