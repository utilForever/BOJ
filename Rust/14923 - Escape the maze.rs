...
fn explore(
    ...
    queue.push_back((y_start, x_start, 0));
    visited[y_start][x_start][0] = 0;

    while !queue.is_empty() {
        let (y, x, num_break) = queue.pop_front().unwrap();

        if y == y_end && x == x_end {
            return visited[y][x][num_break];
        }
...
fn main() {
    ...
    let (hy, hx) = (scan.token::<usize>() - 1, scan.token::<usize>() - 1);
    let (ey, ex) = (scan.token::<usize>() - 1, scan.token::<usize>() - 1);
    ...
    writeln!(
        out,
        "{}",
        explore(&maze, &mut visited, n, m, hy, hx, ey, ex)
    )
    .unwrap();
}
