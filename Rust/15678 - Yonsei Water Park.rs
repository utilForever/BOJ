...
fn main() {
    ...
    let mut stairs = vec![0; n + 1];

    for i in 1..=n {
        stairs[i] = scan.token::<i64>();
    }
    ...
    for i in 1..=n {
        ret[i] = stairs[i].max(stairs[i] + tree.query((i as i64 - d).max(1) as usize, i).max);
        tree.update(i, ret[i]);
    }

    writeln!(out, "{}", ret[1..].iter().max().unwrap()).unwrap();
}
