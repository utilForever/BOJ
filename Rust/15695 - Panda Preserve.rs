...
// Check if a point p is on the line segment between a and b
fn is_on_segment(a: &Point, b: &Point, p: &Point) -> bool {
    let dist = if a == b {
        // If the segment is actually a single point, calculate the distance from p to a
        (*p - *a).dist()
    } else {
        // Calculate the squared distance from a to b
        let d = (*b - *a).dist2();
        // Project p onto the line, clamping t between 0 and d to stay within the segment
        let t = d.min(((*p - *a).dot(&(*b - *a))).max(0.0));

        // Calculate the scaled distance from p to the closest point on the line segment
        ((*p - *a) * d - (*b - *a) * t).dist() / d
    };

    // If the distance is less than a small epsilon, consider p to be on the segment
    dist <= 1e-9
}

// Check if a ray starting at point p crosses the line segment from a and b
fn is_crossing(p: &Point, a: &Point, b: &Point) -> bool {
    // Check if point p's y-coordinate is between the y-coordinates of a and b
    let is_between = (a.y > p.y) != (b.y > p.y);

    if is_between {
        // Calculate the intersection point of the line segment with the ray
        let slope = (b.x - a.x) as f64 / (b.y - a.y) as f64;
        let intersect = a.x as f64 + (p.y - a.y) as f64 * slope;

        // Check if the intersection is to the right of point p
        return intersect > p.x as f64;
    }

    false
}

// Check if a point p is inside a polygon defined by points
fn is_inside(p: &Point, points: &Vec<Point>) -> bool {
    ...
    for i in 0..points.len() {
        ...
        // Check if p is directly on any of the polygon's edges
        if is_on_segment(&points[i], q, p) {
            return true;
        }

        // Compute the vertical crossing number
        if is_crossing(p, &points[i], q) {
            crossings += 1;
        }
    }

    // Return true if the point is inside the polygon (odd number of crossings)
    crossings % 2 == 1
}

// Define a function to project a point p onto the line defined by a and b
fn project_point_to_line(a: &Point, b: &Point, p: &Point) -> Point {
    let v = *b - *a;
    // Calculate the projection and adjust by the perpendicular vector to find the closest point
    *p - v.perp() * 2.0 * v.cross(&(*p - *a)) / v.dist2()
}

// Define a function to calculate the intersection point of lines a - b and c - d
...
fn main() {
    ...
    let delaunay = triangulate(&points).unwrap().triangles;
    let mut voronoi = Vec::with_capacity(delaunay.len() / 3);
    let mut ret = f64::MIN;

    // Calculate Voronoi centers and check for maximum distance within the polygon
    for i in (0..delaunay.len()).step_by(3) {
        let p1 = &points[delaunay[i]];
        let p2 = &points[delaunay[i + 1]];
        let p3 = &points[delaunay[i + 2]];

        let center = circumcenter(p1, p2, p3).unwrap();
        voronoi.push(center);
    }

    // Determine if the Voronoi center is inside the polygon and calculate distances
    for point in voronoi.iter() {
        if is_inside(&point, &points) {
            let mut val = f64::MAX;

            for p in points.iter() {
                val = val.min((*p - *point).dist2());
            }
    ...
    // Further process each Voronoi point for intersections and update the maximum distance found
    for i in (0..delaunay.len()).step_by(3) {
        let p1 = &voronoi[i / 3];

        for j in 0..3 {
            let p2 = project_point_to_line(
                &points[delaunay[i + j]],
                &points[delaunay[i + ((j + 1) % 3)]],
                p1,
            );

            for k in 0..n {
                let candidate = intersect_lines(p1, &p2, &points[k], &points[(k + 1) % n]);

                if is_on_segment(&points[k], &points[(k + 1) % n], &candidate) {
                    let mut val = f64::MAX;

                    for p in points.iter() {
                        val = val.min((*p - candidate).dist2());
                    }
