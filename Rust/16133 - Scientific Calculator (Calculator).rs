...
impl<'a> Parser<'a> {
    ...
    pub fn parse_input(&mut self) -> i64 {
        // <input> ::= <expr> "="
    ...
    fn parse_term(&mut self) -> i64 {
        ...
        while self.curr_token_is(Token::Asterisk) || self.curr_token_is(Token::Slash) {
            let is_asterisk = self.curr_token_is(Token::Asterisk);
    ...
    fn parse_factor(&mut self) -> i64 {
        ...
        while self.curr_token_is(Token::Caret) {
            self.next_token();
    ...
    fn parse_power(&mut self) -> i64 {
        // <power> ::= "#" <power> | <root>
        if self.curr_token_is(Token::Sharp) {
            self.next_token();

            let ret = f128::from(self.parse_power()).sqrt();

            if ret < f128::from(0.0) {
                let val = ret.ceil().to_string();
                return val.parse::<f64>().unwrap() as i64;
            } else {
                let val = ret.floor().to_string();
                return val.parse::<f64>().unwrap() as i64;
            }
        }
