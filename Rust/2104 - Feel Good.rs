...
fn calculate_max(tree: &mut SegmentTree, left: usize, right: usize) -> i64 {
    ...
    let mut ret = node.sum * node.min.1;

    if left < node.min.0 {
        ret = ret.max(calculate_max(tree, left, node.min.0 - 1));
    }

    if node.min.0 < right {
        ret = ret.max(calculate_max(tree, node.min.0 + 1, right));
    }
