use io::Write;
use std::{collections::BTreeMap, f64, fmt, io, str, usize};
...
fn main() {
    ...
    for _ in 0..t {
        ...
        let mut point_idxes = BTreeMap::new();

        for i in 0..n {
            let (x, y) = (scan.token::<f64>(), scan.token::<f64>());
            points.push(Point { x, y });
            point_idxes.insert((x as i64, y as i64), i);
        }
        ...
        for i in 0..idx_triangles.len() / 3 {
            triangles.push((
                points[idx_triangles[3 * i]],
                points[idx_triangles[3 * i + 1]],
                points[idx_triangles[3 * i + 2]],
            ));
        }
        ...
        for triangle in triangles {
            if triangle.0 == points[0] {
                let idx1 = point_idxes
                    .get(&(triangle.1.x as i64, triangle.1.y as i64))
                    .unwrap();
                let idx2 = point_idxes
                    .get(&(triangle.2.x as i64, triangle.2.y as i64))
                    .unwrap();

                ret.push(*idx1);
                ret.push(*idx2);
            } else if triangle.1 == points[0] {
                let idx1 = point_idxes
                    .get(&(triangle.0.x as i64, triangle.0.y as i64))
                    .unwrap();
                let idx2 = point_idxes
                    .get(&(triangle.2.x as i64, triangle.2.y as i64))
                    .unwrap();
