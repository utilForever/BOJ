...
fn main() {
    ...
    let (n, prison) = (
        scan.token::<usize>(),
        Point::new(scan.token::<i64>(), scan.token::<i64>(), 0, 0),
    );
    ...
    loop {
        ...
        let point0 = stack_cloned.last().unwrap().clone();
        let mut point1 = stack_cloned.pop().unwrap();
        let mut point2 = stack_cloned.pop().unwrap();
        let direction = calculate_ccw(point1, point2, prison);
        let mut is_inner = true;

        while !stack_cloned.is_empty() {
            point1 = point2;
            point2 = stack_cloned.pop().unwrap();

            if direction != calculate_ccw(point1, point2, prison) {
                is_inner = false;
                break;
            }
        }

        if direction != calculate_ccw(point2, point0, prison) {
            is_inner = false;
        }
        ...
        if is_inner {
            ...
            for point in stack.iter() {
                points.remove(points.iter().position(|x| x == point).unwrap());
            }
        } else {
            break;
        }
