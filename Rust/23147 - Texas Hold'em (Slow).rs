...
fn main() {
    ...
    let convert_card = |raw_suit: &str, raw_card: &str| -> Card {
    ...
    for _ in 0..t {
        let (w, m) = (scan.token::<i64>(), scan.token::<i64>());
        ...
        for card in cards_alice.iter_mut().take(2) {
            let (raw_suit, raw_card) = (scan.token::<String>(), scan.token::<String>());
            *card = convert_card(&raw_suit, &raw_card);
        }
                ...
                for k in j + 1..46 {
                    ...
                    for l in k + 1..47 {
                        ...
                        for m in l + 1..48 {
                            ...
                            let mut alice_category_max =
                                Category::HighCard(Hand::new(cards_alice.to_vec()));
                            let mut bob_category_max =
                                Category::HighCard(Hand::new(cards_bob.to_vec()));
        ...
        if cnt_win_alice == cnt_win_bob {
            ...
            let fold = -w * (cnt_win_alice + cnt_win_bob + cnt_draw);
            let all_in = -(w + m) * (cnt_win_alice - cnt_win_bob);
            ...
            if fold > all_in {
                ...
                let denominator = cnt_win_alice + cnt_win_bob + cnt_draw;
            ...
            let fold = -w * (cnt_win_alice + cnt_win_bob + cnt_draw);
            let all_in = -(w + m) * (cnt_win_bob - cnt_win_alice);
