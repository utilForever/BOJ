...
fn main() {
    ...
    for _ in 0..t {
        ...
        let mut nums: BTreeMap<i64, Vec<usize>> = BTreeMap::new();
        let mut tree = SegmentTree::new(n + 1);

        for i in 1..=n {
            let num = scan.token::<i64>();

            nums.entry(num).and_modify(|x| x.push(i)).or_insert(vec![i]);
            tree.update(i, num);
        }

        let mut ret = true;

        for (num, idxes) in nums {
            if idxes.len() == 1 {
                continue;
            }

            let idx_min = *idxes.iter().min().unwrap();
            let idx_max = *idxes.iter().max().unwrap();

            if num < tree.query(idx_min, idx_max).max {
                ret = false;
                break;
            }
        }
