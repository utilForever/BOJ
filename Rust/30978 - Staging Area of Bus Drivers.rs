...
impl LazySegmentTree {
    ...
    fn propagate(&mut self, node: usize, start: usize, end: usize) {
        ...
        self.data[node].max |= self.lazy_or[node];
        self.data[node].max &= !self.lazy_and[node];
...
fn main() {
    ...
    for _ in 0..q {
        ...
        if command == 1 {
            ...
            tree.update_and(a, b, !val);
        } else if command == 3 {
            let (a, m) = (scan.token::<usize>(), scan.token::<i64>());
            let mut val = 0;
            ...
            tree.update_and(a, a, !0);
            tree.update_or(a, a, val);
        } else if command == 4 {
            let (a, b) = (scan.token::<usize>(), scan.token::<usize>());
            let val_a = tree.query_min(a, a);
            let val_b = tree.query_min(b, b);

            tree.update_and(a, a, !0);
            tree.update_or(a, a, val_b);
            tree.update_and(b, b, !0);
            tree.update_or(b, b, val_a);
        } else if command == 5 {
            let (a, b) = (scan.token::<usize>(), scan.token::<usize>());
            let val = tree.query_min(a, b);
            ...
            let (a, b) = (scan.token::<usize>(), scan.token::<usize>());
            let val_min = tree.query_min(a, b);
            let val_max = tree.query_max(a, b);
