...
impl SegmentTree {
    ...
    fn merge(a: &Node, b: &Node) -> Node {
        Node {
            left_max: if a.is_red {
                a.left_max + b.left_max
            } else {
                a.left_max
            },
            right_max: if b.is_red {
                a.right_max + b.right_max
            } else {
                b.right_max
            },
    ...
    fn update_internal(
        ...
        if node_start == node_end {
            if is_red {
                self.data[node].is_red = true;
                self.data[node].left_max = 1;
                self.data[node].right_max = 1;
                self.data[node].total_max = 1;
            } else {
                self.data[node].is_red = false;
                self.data[node].left_max = 0;
                self.data[node].right_max = 0;
                self.data[node].total_max = 0;
            }
        ...
        if val <= mid as i64 {
            self.update_internal(val, is_red, node * 2, node_start, mid);
        } else {
            self.update_internal(val, is_red, node * 2 + 1, mid + 1, node_end);
        }
...
fn main() {
    ...
    let mut points = vec![(0, 0, false); n];
