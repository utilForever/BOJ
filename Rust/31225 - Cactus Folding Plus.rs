    ...
    if let Some(adj) = adjacent[point_end].get(&vcc) {
        for &(e, v) in adj {
            if connectivity.vcc[e] == vcc && !set.contains(&v) && !visited[e] && !visited[e ^ 1] {
                ...
                ret[vcc].push((connectivity.graph.cost[e], e));
                break;
            } else if connectivity.vcc[e] == vcc && !visited[e] && !visited[e ^ 1] {
                visited[e] = true;
                visited[e ^ 1] = true;
                ret[connectivity.vcc[e]].push((connectivity.graph.cost[e], e));
            }
        }
    }
}
...
fn process_dfs2(
    ...
    positions[index] = cost_curr;
    visited[index] = true;

    for (e, v) in connectivity.graph.adj_list(index) {
        if !visited[v] {
            process_dfs2(
                connectivity,
                positions,
...
fn main() {
    ...
    for i in 0..connectivity.graph.num_e() {
        ...
        ret[connectivity.vcc[i]].push((connectivity.graph.cost[i], i));
    ...
    let mut positions = vec![0; connectivity.graph.num_v()];
    let mut costs = vec![0; connectivity.graph.num_e()];
    let mut is_sat = true;

    for i in 1..=connectivity.num_vcc {
        if ret[i].len() <= 1 {
            costs[ret[i][0].1] = ret[i][0].0 as i64;
            costs[ret[i][0].1 ^ 1] = ret[i][0].0 as i64;
        } else {
            if ret[i].iter().map(|&x| x.0).sum::<i64>() % 2 == 1 {
                is_sat = false;
                break;
            }

            let subset_sum = subset_sum(
                &ret[i].iter().map(|&x| x.0 as i64).collect::<Vec<_>>(),
                ret[i].iter().map(|&x| x.0).sum::<i64>() / 2,
            );
            ...
            for j in 0..ret[i].len() {
                if subset_sum[j] {
                    costs[ret[i][j].1] = -ret[i][j].0 as i64;
                    costs[ret[i][j].1 ^ 1] = -ret[i][j].0 as i64;
                } else {
                    costs[ret[i][j].1] = ret[i][j].0 as i64;
                    costs[ret[i][j].1 ^ 1] = ret[i][j].0 as i64;
                }
    ...
    let mut visited = vec![false; connectivity.graph.num_v()];
