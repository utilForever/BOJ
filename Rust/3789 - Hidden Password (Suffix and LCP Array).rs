...
fn main() {
    ...
    for _ in 0..t {
        ...
        let suffix_array = build_suffix_array(&s_new);
        let lcp_array = build_lcp_array(&s_new, &suffix_array);
        let mut ret: Option<usize> = None;

        writeln!(out, "{:?}", suffix_array).unwrap();
        writeln!(out, "{:?}", lcp_array).unwrap();
        ...
        for i in 0..s_new.len() {
            match ret {
                Some(val) => {
                    ...
                    ret = Some(val.min(suffix_array[i]));
                }
                None => {
                    if suffix_array[i] < n {
                        ret = Some(suffix_array[i]);
                    }
                }
            }
        }
