...
fn in_triangle(p: &Point, a: &Point, b: &Point, c: &Point) -> bool {
    ...
    let is_neg = d1 < 0.0 || d2 < 0.0 || d3 < 0.0;
    let is_pos = d1 > 0.0 || d2 > 0.0 || d3 > 0.0;
...
fn main() {
    ...
    for _ in 0..t {
        ...
        for i in 0..arcs.len() {
            ...
            let dist = (arcs[i].0.x - arcs[i].1.x).hypot(arcs[i].0.y - arcs[i].1.y);

            vec.push((dist / 2.0, faces[i][0], faces[i][1]));
        }
