...
fn main() {
    ...
    for i in 0..n {
        areas[i] = (scan.token::<i64>(), scan.token::<i64>());
    }
    ...
    areas_new.push(areas[0]);

    for i in 1..n {
        if areas_new.last().unwrap().1 >= areas[i].1 {
            continue;
        }

        areas_new.push(areas[i]);
    }
    ...
    for i in 1..=n {
        insert(&mut stack, &mut size, areas_new[i - 1].1, cost[i - 1]);
        cost[i] = query(&stack, &size, &mut last, areas_new[i - 1].0);
    }
