...
fn main() {
    ...
    for i in 1..s.len() {
        if (suffix_array[i - 1] > a.len() && suffix_array[i] < a.len())
            || (suffix_array[i - 1] < a.len() && suffix_array[i] > a.len())
        {
            if ret.0 < lcp_array[i] {
                ret = (
                    lcp_array[i],
                    s[suffix_array[i]..suffix_array[i] + lcp_array[i]].to_string(),
                );
            }
        }
    }
