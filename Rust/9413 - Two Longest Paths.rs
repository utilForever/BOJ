...
fn main() {
    ...
    for _ in 0..t {
        ...
        let mut mcmf = MCMF::new(2 * n + 4, 2 * n, 2 * n + 1);

        for _ in 0..m {
            let (a, b) = (scan.token::<usize>(), scan.token::<usize>());
            mcmf.add_edge(2 * (a - 1) + 1, 2 * (b - 1), 1, 0);
        }

        for i in 0..n {
            mcmf.add_edge(2 * i, 2 * i + 1, 1, -1);
            mcmf.add_edge(2 * n + 2, 2 * i, 1, 0);
            mcmf.add_edge(2 * i + 1, 2 * n + 3, 1, 0);
        }
